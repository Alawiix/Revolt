import os ,sys ,json ,ctypes ,subprocess ,math ,threading ,urllib .request ,urllib .parse ,urllib .error 
...
APP_DATA_DIR =os .path .join (os .path .expanduser ("~"),"Revolt Data")
try :
    os .makedirs (APP_DATA_DIR ,exist_ok =True )
except Exception :
    pass

CONFIG_FILE =os .path .join (APP_DATA_DIR ,"revolt_config.json")
REMOTE_CACHE_FILE =os .path .join (APP_DATA_DIR ,"revolt_remote_cache.json")
CUSTOM_FEEDS_FILE =os .path .join (APP_DATA_DIR ,"revolt_custom_feeds.json")
CUSTOM_DNS_FILE =os .path .join (APP_DATA_DIR ,"revolt_custom_dns.json")
ALLOWLIST_FILE =os .path .join (APP_DATA_DIR ,"revolt_allowlist.json")
LOCK_FILE =os .path .join (APP_DATA_DIR ,"revolt_lock.json")
VPN_CONFIG_FILE =os .path .join (APP_DATA_DIR ,"revolt_vpn_config.json")
VPN_PID_FILE =os .path .join (APP_DATA_DIR ,"revolt_vpn.pid")
VPN_STATE_FILE =os .path .join (APP_DATA_DIR ,"revolt_vpn_state.json")
VPN_LOG_FILE =os .path .join (APP_DATA_DIR ,"revolt_vpn_openvpn.log")
VPN_ROTATE_CONFIG_FILE =os .path .join (APP_DATA_DIR ,"revolt_vpn_rotate.json")
DNS_STATE_FILE =os .path .join (APP_DATA_DIR ,"revolt_dns_state.json")
APP_LOG_FILE =os .path .join (APP_DATA_DIR ,"revolt_app.log")

CUSTOM_ICONS_DIR =os .path .join (APP_DATA_DIR ,"custom_icons")
_APP_NAME ="Revolt"
...
_SHORTCUT_MARKER =os .path .join (APP_DATA_DIR ,".revolt_shortcut_created")
_ONBOARDING_MARKER =os .path .join (APP_DATA_DIR ,".revolt_onboarding_done")
...
CUSTOM_ICON_MAX_BYTES =8 *1024 *1024 
CUSTOM_ICON_RASTER_EXT ={".png",".jpg",".jpeg",".gif",".bmp",".ico",".webp"}
CUSTOM_ICON_ALLOWED_EXT =CUSTOM_ICON_RASTER_EXT |({".svg"}if _SVG_ICONS_AVAILABLE else set ())

_PROTECTED_ICON_BASENAMES ={
os .path .basename (ICON_PATH ).lower (),
os .path .basename (ICON_PNG_PATH ).lower (),
os .path .basename (CHIP_ICON_PATH ).lower (),
os .path .basename (CREDIT_ICON_PATH ).lower (),
}

def _ensure_custom_icons_dir ()->str :
    try :
...
def is_custom_image_icon (value )->bool :
    """True if a stored icon value refers to a user-uploaded image rather
    than a plain emoji/character glyph."""
    return isinstance (value ,str )and value .startswith ("img:")

def resolve_custom_icon_path (ref :str )->str |None :
    """Resolve an 'img:<filename>' reference to a real path, strictly
    confined to CUSTOM_ICONS_DIR. Returns None for anything else, including
    any reference (however contrived) that would resolve to one of
    Revolt's protected brand-asset filenames."""
    if not is_custom_image_icon (ref ):
        return None 
    fname =ref [4 :]
    if not fname or os .path .basename (fname )!=fname :
        return None 
    if fname .lower ()in _PROTECTED_ICON_BASENAMES :
        return None 
    base =os .path .abspath (_ensure_custom_icons_dir ())
    path =os .path .abspath (os .path .join (base ,fname ))
    if os .path .commonpath ([base ,path ])!=base :
        return None 
    return path if os .path .exists (path )else None 

def delete_custom_icon_file (ref :str ):
    path =resolve_custom_icon_path (ref )
    if not path :
        return 
    try :
        os .remove (path )
...
def import_custom_topic_icon (src_path :str )->str |None :
    """Copy/convert a user-picked image file into CUSTOM_ICONS_DIR under a
    fresh randomized filename and return an 'img:<filename>' reference, or
    None on failure. This only ever reads src_path and only ever writes
    inside CUSTOM_ICONS_DIR — it cannot touch Revolt's bundled branding."""
    if not src_path or not os .path .exists (src_path )or not os .path .isfile (src_path ):
        return None 
    ext =os .path .splitext (src_path )[1 ].lower ()
    if ext not in CUSTOM_ICON_ALLOWED_EXT :
        return None 
    try :
        if os .path .getsize (src_path )>CUSTOM_ICON_MAX_BYTES :
            return None 
    except Exception :
        return None 
    ...
    try :
        if ext ==".svg":
            if not _SVG_ICONS_AVAILABLE :
                return None 
            png_bytes =_cairosvg .svg2png (url =src_path ,
            output_width =CUSTOM_ICON_MAX_DIM ,output_height =CUSTOM_ICON_MAX_DIM )
            img =_PILImage .open (io .BytesIO (png_bytes )).convert ("RGBA")
        else :
            img =_PILImage .open (src_path )
            img .load ()
            img =img .convert ("RGBA")
        img .thumbnail ((CUSTOM_ICON_MAX_DIM ,CUSTOM_ICON_MAX_DIM ),_PILImage .LANCZOS )
        _ensure_custom_icons_dir ()
        fname =f"icon_{secrets .token_hex (8 )}.png"
        dest =os .path .join (CUSTOM_ICONS_DIR ,fname )
        img .save (dest ,"PNG")
        return f"img:{fname }"
    except Exception :
        return None 

_CUSTOM_ICON_PHOTO_CACHE ={}

def render_custom_icon_photo (path :str |None ,size :int ):
    """Load a custom-icon file as a square, centered tk.PhotoImage, cached
    by (path, size, mtime). Returns None if unavailable."""
    if not _PIL_AVAILABLE or not path or not os .path .exists (path ):
        return None 
    try :
        ...
        mtime =0 
    key =(path ,size ,mtime )
    cached =_CUSTOM_ICON_PHOTO_CACHE .get (key )
    ...
    try :
        ...
        photo =_PILImageTk .PhotoImage (canvas )
        _CUSTOM_ICON_PHOTO_CACHE [key ]=photo 
        return photo 
    except Exception :
        return None 
...
# Ali, Ahmed, Motaki proudly made this project.
...
_CLAMSCAN_NAMES =["clamscan","clamscan.exe"]
_FRESHCLAM_NAMES =["freshclam","freshclam.exe"]
_CLAMAV_WIN_DIRS =[
r"C:\Program Files\ClamAV",
r"C:\Program Files (x86)\ClamAV",
r"C:\ClamAV",
r"C:\clamav-1.4.1.win.x64",
]
_CLAMAV_POSIX_DIRS =[
...
def _find_clamav_tool (names ,extra_dirs =None )->str |None :
    for name in names :
        found =shutil .which (name )
        if found :
            return found 
    search_dirs =list (_CLAMAV_WIN_DIRS if os .name =="nt"else _CLAMAV_POSIX_DIRS )
    if extra_dirs :
...
def find_clamscan ()->str |None :
    return _find_clamav_tool (_CLAMSCAN_NAMES )

def find_freshclam ()->str |None :
    clam =find_clamscan ()
    extra =[os .path .dirname (clam )]if clam else None 
    return _find_clamav_tool (_FRESHCLAM_NAMES ,extra_dirs =extra )

def clamav_available ()->bool :
    return find_clamscan ()is not None 

_CLAM_FOUND_RE =re .compile (r"^(.*):\s+(.+?)\s+FOUND\s*$")

def _db_dir_has_signatures (db_dir :str |None )->bool :
    if not db_dir or not os .path .isdir (db_dir ):
        return False 
    try :
        for name in os .listdir (db_dir ):
            if name .lower ().endswith ((".cvd",".cld")):
                return True 
    except Exception :
        pass 
    return False 

def resolve_clamav_db_dir ()->str |None :
    db_dir =get_clamav_db_dir ()
    if db_dir :
        return db_dir 
    exe =find_clamscan ()
    if not exe :
        return None 
    if os .name =="nt":
        return os .path .join (os .path .dirname (exe ),"database")
    return "/var/lib/clamav"

def run_clamav_scan (path :str ,line_callback =None ,stop_event =None ):

    exe =find_clamscan ()
    if not exe :
        return {"ok":False ,"error":"ClamAV (clamscan) is not installed on this system.",
        "threats":[],"scanned":0 ,"infected":0 ,"raw":""}

    db_dir =resolve_clamav_db_dir ()
    if not _db_dir_has_signatures (db_dir ):
        return {"ok":False ,
        "error":"No virus definitions found yet. Click \"Update Definitions\" "
        "first, then try scanning again.",
        "threats":[],"scanned":0 ,"infected":0 ,"raw":""}

    cmd =[exe ,"-r","--stdout","-i",
    "--max-filesize=2147483647","--max-scansize=2147483647",
    "--max-recursion=32","--max-files=0"]
    if pua_detection_enabled ():
        cmd +=["--detect-pua=yes"]
    if db_dir and os .path .isdir (db_dir ):
        cmd +=["--database",db_dir ]
    custom_dir =ensure_custom_defs_dir ()
    if list_custom_defs ():
        cmd +=["--database",custom_dir ]
    cmd .append (path )
    ...
    try :
        ...
        return {"ok":False ,"error":f"Couldn't start clamscan: {e }",
        "threats":[],"scanned":0 ,"infected":0 ,"raw":""}
    ...
    try :
        for raw_line in proc .stdout :
            ...
            if not line :
                continue 
            lines .append (line )
            m =_CLAM_FOUND_RE .match (line )
            if m :
                threats .append ((m .group (1 ).strip (),m .group (2 ).strip ()))
            if line_callback :
                try :
                    line_callback (line ,bool (m ))
                except Exception :
                    pass 
    ...
    m =re .search (r"Scanned files:\s*(\d+)",raw )
    if m :
        scanned =int (m .group (1 ))
    m =re .search (r"Infected files:\s*(\d+)",raw )
    if m :
        infected =int (m .group (1 ))

    ok =proc .returncode in (0 ,1 )
    return {"ok":ok ,"error":None if ok else "clamscan reported an error — see the log below.",
    "threats":threats ,"scanned":scanned ,"infected":infected ,"raw":raw }

CUSTOM_DEFS_DIR =os .path .join (APP_DATA_DIR ,"custom_definitions")
_CUSTOM_DEF_EXTS =(".hdb",".hsb",".ndb",".ldb",".yar",".yara")

def ensure_custom_defs_dir ()->str :
    os .makedirs (CUSTOM_DEFS_DIR ,exist_ok =True )
    return CUSTOM_DEFS_DIR 

def _hdb_hsb_line_ok (line :str )->bool :
    parts =line .split (":")
    if len (parts )<2 :
        return False 
    hexhash =parts [0 ].strip ()
    return len (hexhash )in (32 ,40 ,64 )and all (c in "0123456789abcdefABCDEF"for c in hexhash )

def _ndb_line_ok (line :str )->bool :
    parts =line .split (":")
    return len (parts )>=4 

def validate_signature_file (path :str )->tuple [bool ,str ]:
    ext =os .path .splitext (path )[1 ].lower ()
    if ext not in _CUSTOM_DEF_EXTS :
        return False ,f"Unsupported extension \"{ext }\" — expected one of {', '.join (_CUSTOM_DEF_EXTS )}."
    try :
        with open (path ,"r",encoding ="utf-8",errors ="ignore")as f :
            text_lines =[l .strip ()for l in f if l .strip ()]
    except Exception as e :
        return False ,f"Couldn't read the file: {e }"
    if not text_lines :
        return False ,"File is empty."

    if ext in (".hdb",".hsb"):
        bad =[l for l in text_lines if not _hdb_hsb_line_ok (l )]
        if bad :
            return False ,("Doesn't look like valid ClamAV hash-signature "
            "(.hdb/.hsb) syntax — expected lines like "
            "\"<md5/sha1/sha256>:<size>:<name>\".")
    elif ext ==".ndb":
        bad =[l for l in text_lines if not _ndb_line_ok (l )]
        if bad :
            return False ,("Doesn't look like valid ClamAV extended-signature "
            "(.ndb) syntax — expected lines like "
            "\"Name:TargetType:Offset:HexSignature\".")
    elif ext ==".ldb":
        bad =[l for l in text_lines if l .count (";")<2 ]
        if bad :
            return False ,"Doesn't look like valid ClamAV logical-signature (.ldb) syntax."
    elif ext in (".yar",".yara"):
        if not any (l .startswith ("rule ")or " rule "in l for l in text_lines ):
            return False ,"Doesn't look like a valid YARA rule (no \"rule\" block found)."
    return True ,"OK"

def list_custom_defs ()->list [dict ]:
    d =ensure_custom_defs_dir ()
    out =[]
    try :
        for name in sorted (os .listdir (d )):
            ...
            if not os .path .isfile (p ):
                continue 
            if os .path .splitext (name )[1 ].lower ()not in _CUSTOM_DEF_EXTS :
                continue 
            try :
                sig_count =sum (1 for l in open (p ,"r",encoding ="utf-8",errors ="ignore")if l .strip ())
            except Exception :
                sig_count =0 
            out .append ({"name":name ,"path":p ,
            "size":os .path .getsize (p ),"signatures":sig_count })
    except Exception :
        pass 
    return out 

def add_custom_def_file (src_path :str )->tuple [bool ,str ]:
    ok ,msg =validate_signature_file (src_path )
    if not ok :
        return False ,msg 
    d =ensure_custom_defs_dir ()
    ...
    try :
        ...
        return False ,f"Couldn't copy the file into Revolt's custom-definitions folder: {e }"
    return True ,f"Added {os .path .basename (src_path )}."

def remove_custom_def_file (name :str )->tuple [bool ,str ]:
    d =ensure_custom_defs_dir ()
    p =os .path .join (d ,name )
    try :
        if os .path .isfile (p ):
...
SANESECURITY_MIRROR_LIST_URL ="https://www.sanesecurity.com/mirrors.txt"
_SANESECURITY_FALLBACK_MIRRORS =[
"http://mirror.rollernet.us/sanesecurity/",
"http://sanesecurity.mirror.ndchost.com/",
]

SUGGESTED_DEFS =[
{"id":"winnow_extended_malware","filename":"winnow_extended_malware.hdb",
"name":"Extended Malware & Trojan Hashes",
"category":"Advanced Trojans & Malware","source":"sanesecurity",
"desc":"Community-maintained hash signatures (Sanesecurity) covering "
"trojans and malware families not yet in the official ClamAV database."},
{"id":"winnow_malware_links","filename":"winnow_malware_links.ndb",
"name":"Malicious Links & Dropper Scripts",
"category":"Advanced Trojans & Malware","source":"sanesecurity",
"desc":"Pattern signatures (Sanesecurity) for known malicious download "
"links and trojan-dropper scripts."},
{"id":"rogue","filename":"rogue.hdb",
"name":"Rogue / Fake-Security Malware",
"category":"Advanced Trojans & Malware","source":"sanesecurity",
"desc":"Hashes (Sanesecurity) for rogue antivirus and fake security-alert malware."},
{"id":"phish","filename":"phish.ndb",
"name":"Phishing & Credential-Harvesting Kits",
"category":"Phishing / Social-Engineering","source":"sanesecurity",
"desc":"Detects phishing-page kits and credential-harvesting scripts — "
"the kind of tooling behind account-takeover and social-media "
"hijacking attacks (Sanesecurity)."},
{"id":"scam","filename":"scam.ndb",
"name":"Scam & Social-Engineering Pages",
"category":"Phishing / Social-Engineering","source":"sanesecurity",
"desc":"Detects known scam and social-engineering web-page kits (Sanesecurity)."},
]

USER_SUGGESTED_DEFS_FILE =os .path .join (APP_DATA_DIR ,"revolt_user_suggested_defs.json")

def load_user_suggested_defs ()->list [dict ]:
    try :
        with open (USER_SUGGESTED_DEFS_FILE ,"r",encoding ="utf-8")as f :
            data =json .load (f )
        return data if isinstance (data ,list )else []
    except Exception :
        return []

def save_user_suggested_defs (entries :list [dict ]):
    try :
        os .makedirs (APP_DATA_DIR ,exist_ok =True )
        with open (USER_SUGGESTED_DEFS_FILE ,"w",encoding ="utf-8")as f :
            json .dump (entries ,f ,indent =2 )
    except Exception :
        pass 

def all_suggested_defs ()->list [dict ]:
    return SUGGESTED_DEFS +load_user_suggested_defs ()

def add_user_suggested_list (name :str ,category :str ,url :str )->tuple [bool ,str ]:
    name =name .strip ()
    url =url .strip ()
    if not name :
        return False ,"Give the list a name."
    if not (url .startswith ("http://")or url .startswith ("https://")):
        return False ,"URL must start with http:// or https://"
    filename =os .path .basename (url .split ("?")[0 ])
    if os .path .splitext (filename )[1 ].lower ()not in _CUSTOM_DEF_EXTS :
        return False ,(f"The URL must point directly at a {', '.join (_CUSTOM_DEF_EXTS )} "
        "signature file.")
    entries =load_user_suggested_defs ()
    entry_id ="user_"+re .sub (r"[^a-z0-9]+","_",name .lower ()).strip ("_")
    if any (e ["id"]==entry_id for e in entries )or any (e ["id"]==entry_id for e in SUGGESTED_DEFS ):
        return False ,"A suggested list with that name already exists."
    entries .append ({"id":entry_id ,"filename":filename ,"name":name ,
    "category":category .strip ()or "Custom List","source":"direct","url":url ,
    "desc":f"User-added signature feed from {url }."})
    save_user_suggested_defs (entries )
    return True ,f"Added \"{name }\" to Suggested Definitions."

def remove_user_suggested_list (entry_id :str )->tuple [bool ,str ]:
    entries =load_user_suggested_defs ()
    kept =[e for e in entries if e ["id"]!=entry_id ]
    if len (kept )==len (entries ):
        return False ,"That list wasn't found."
    save_user_suggested_defs (kept )
    return True ,"List removed from Suggested Definitions."

AV_OPTIONS_FILE =os .path .join (APP_DATA_DIR ,"revolt_av_options.json")

def load_av_options ()->dict :
    try :
        with open (AV_OPTIONS_FILE ,"r",encoding ="utf-8")as f :
            return json .load (f )
    except Exception :
        return {}

def save_av_options (opts :dict ):
    try :
        os .makedirs (APP_DATA_DIR ,exist_ok =True )
        with open (AV_OPTIONS_FILE ,"w",encoding ="utf-8")as f :
            json .dump (opts ,f )
    except Exception :
        pass 

def pua_detection_enabled ()->bool :
    return bool (load_av_options ().get ("detect_pua"))

def set_pua_detection (enabled :bool ):
    opts =load_av_options ()
    opts ["detect_pua"]=enabled 
    save_av_options (opts )


def suggested_def_installed (entry :dict )->bool :
    return os .path .isfile (os .path .join (ensure_custom_defs_dir (),entry ["filename"]))

def _sanesecurity_mirrors ()->list [str ]:
    mirrors =[]
    try :
        with urllib .request .urlopen (SANESECURITY_MIRROR_LIST_URL ,timeout =6 )as resp :
        ...
        pass 
    return mirrors or list (_SANESECURITY_FALLBACK_MIRRORS )

def download_suggested_def (entry :dict )->tuple [bool ,str ]:
    if entry .get ("source")=="direct":
        url =entry .get ("url","")
        try :
            req =urllib .request .Request (url ,headers ={"User-Agent":"Revolt/1.0"})
            with urllib .request .urlopen (req ,timeout =15 )as resp :
                data =resp .read ()
            tmp_dir =tempfile .mkdtemp (prefix ="revolt_defs_")
            tmp_path =os .path .join (tmp_dir ,entry ["filename"])
            with open (tmp_path ,"wb")as f :
                f .write (data )
            ok ,msg =add_custom_def_file (tmp_path )
            try :
                shutil .rmtree (tmp_dir ,ignore_errors =True )
            except Exception :
                pass 
            return (True ,f"Added \"{entry ['name']}\".")if ok else (False ,msg )
        except Exception as e :
            return False ,f"Couldn't download \"{entry ['name']}\": {e }"

    mirrors =_sanesecurity_mirrors ()
    last_err ="No mirrors available."
    for base in mirrors :
        ...
        try :
            ...
            try :
                ...
                pass 
            if ok :
                return True ,f"Added \"{entry ['name']}\"."
            last_err =msg 
        except Exception as e :
            last_err =str (e )
            continue 
    return False ,(f"Couldn't download \"{entry ['name']}\" from any known "
    f"mirror ({last_err }). The mirror list or file name may have "
    "changed — check https://www.sanesecurity.com/usage/ for current details.")


def check_leakcheck_public (query :str )->tuple [bool ,int ,list ,list ,str ]:
    """Keyless breach lookup via LeakCheck's free public API. Works for
    both email addresses and usernames (auto-detected server-side).
    Returns (ok, found_count, sources, fields, err):
      - sources: list of {"name":.., "date":..} dicts — which breaches.
      - fields: list of category names (e.g. "Passwords", "Phone numbers")
        describing WHAT KIND of data was exposed in those breaches.
    Never the actual leaked values themselves (that's Pro-tier only) —
    just which breaches, and what categories of info they contained."""
    query =(query or "").strip ()
    if not query :
        return False ,0 ,[],[],"Enter something to check."
    url =f"https://leakcheck.io/api/public?check={urllib .parse .quote (query )}"
    req =urllib .request .Request (url ,headers ={"User-Agent":"Revolt-Breach-Check"})
    try :
        with urllib .request .urlopen (req ,timeout =12 )as resp :
            data =json .loads (resp .read ().decode ("utf-8"))
        if not data .get ("success",False ):
            return False ,0 ,[],[],data .get ("error","LeakCheck couldn't process that lookup.")
        found =int (data .get ("found",0 )or 0 )
        sources =data .get ("sources",[])or []
        fields =data .get ("fields",[])or []
        return True ,found ,sources ,fields ,""
    except urllib .error .HTTPError as e :
        if e .code ==429 :
            return False ,0 ,[],[],"Rate limited by LeakCheck — wait a moment and try again."
        return False ,0 ,[],[],f"LeakCheck returned an error (HTTP {e .code })."
    except Exception as e :
        return False ,0 ,[],[],f"Couldn't reach LeakCheck: {e }"

def check_email_breaches (email :str )->tuple [bool ,int ,list ,list ,str ]:
    email =(email or "").strip ()
    if not email or "@"not in email :
        return False ,0 ,[],[],"Enter a valid email address."
    return check_leakcheck_public (email )

def check_username_breaches (username :str )->tuple [bool ,int ,list ,list ,str ]:
    username =(username or "").strip ()
    if not username :
        return False ,0 ,[],[],"Enter a username to check."
    return check_leakcheck_public (username )


def check_password_pwned (password :str )->tuple [bool ,int ,str ]:
    """k-anonymity password check — only a 5-char hash prefix is sent."""
    if not password :
        return False ,0 ,"Enter a password to check."
    ...
    try :
        ...
        return False ,0 ,f"Couldn't reach the Pwned Passwords service: {e }"

def check_domain_breaches (domain :str )->tuple [bool ,list ,str ]:
    domain =domain .strip ().lower ()
    if not domain or "."not in domain :
        return False ,[],"Enter a domain, e.g. \"adobe.com\"."
    url =f"https://haveibeenpwned.com/api/v3/breaches?domain={urllib .parse .quote (domain )}"
    req =urllib .request .Request (url ,headers ={"User-Agent":"Revolt-Breach-Check"})
    try :
        with urllib .request .urlopen (req ,timeout =12 )as resp :
            data =json .loads (resp .read ().decode ("utf-8"))
        ...
        if e .code ==404 :
            return True ,[],""
        return False ,[],f"HIBP returned an error (HTTP {e .code })."
    except Exception as e :
        return False ,[],f"Couldn't reach HIBP: {e }"

def get_clamav_db_dir ()->str |None :
    exe =find_freshclam ()or find_clamscan ()
    if not exe :
        return None 
    for path in _freshclam_conf_candidates (exe ):
...
def _freshclam_conf_candidates (exe :str )->list [str ]:
    d =os .path .dirname (exe )
    if os .name =="nt":
        return [os .path .join (d ,"freshclam.conf")]
    prefix =os .path .dirname (d )
    return [
    "/etc/freshclam.conf",
    "/etc/clamav/freshclam.conf",
    os .path .join (prefix ,"etc","freshclam.conf"),
    os .path .join (prefix ,"etc","clamav","freshclam.conf"),
    ]

def _default_db_dir (conf_path :str )->str :
    if os .name =="nt":
        return os .path .join (os .path .dirname (conf_path ),"database")
    return "/var/lib/clamav"

def _ensure_freshclam_conf (exe :str ,line_callback =None )->str |None :
    candidates =_freshclam_conf_candidates (exe )
        ...
        try :
            if os .path .isfile (sample ):
                with open (sample ,"r",encoding ="utf-8",errors ="ignore")as f :
                    lines =f .readlines ()
                out_lines =[l for l in lines 
                if not l .strip ().lower ().startswith ("example")]
                text ="".join (out_lines )
                if "DatabaseDirectory"not in text :
                    text +=f"\nDatabaseDirectory {_default_db_dir (path )}\n"
                os .makedirs (conf_dir ,exist_ok =True )
                db_dir =_default_db_dir (path )
                os .makedirs (db_dir ,exist_ok =True )
                with open (path ,"w",encoding ="utf-8")as f :
                    f .write (text )
                if line_callback :
                    line_callback (f"Created {path } from the bundled sample config.")
                return path 
        except Exception :
            continue 
    ...
    try :
        ...
        with open (path ,"w",encoding ="utf-8")as f :
            ...
            "Checks 24\n")
        if line_callback :
            line_callback (f"Created a default {path }.")
        return path 
    except Exception as e :
        if line_callback :
            line_callback (f"Couldn't create freshclam.conf: {e }")
        return None 

def run_freshclam_update (line_callback =None ):

    exe =find_freshclam ()
    if not exe :
        return False ,"freshclam (ClamAV's updater) was not found on this system."

    conf_path =_ensure_freshclam_conf (exe ,line_callback )
    cmd =[exe ]
    if conf_path :
        cmd +=["--config-file",conf_path ]
    try :
        proc =subprocess .Popen (cmd ,stdout =subprocess .PIPE ,stderr =subprocess .STDOUT ,
        text =True ,bufsize =1 ,universal_newlines =True ,
        startupinfo =_si_hidden (),
        ...
        msg ="Virus definitions are up to date."if ok else "\n".join (out_lines [-6 :])
        return ok ,msg 
    except Exception as e :
        return False ,f"Couldn't run freshclam: {e }"


class _InstallCancelled (Exception ):
    pass 

_CLAMAV_GH_REPO ="Cisco-Talos/clamav"

def _gh_latest_release (repo :str )->dict |None :
    try :
        req =urllib .request .Request (
        f"https://api.github.com/repos/{repo }/releases/latest",
        headers ={"User-Agent":"Revolt-App","Accept":"application/vnd.github+json"})
        with urllib .request .urlopen (req ,timeout =20 )as resp :
            return json .loads (resp .read ().decode ("utf-8","ignore"))
    except Exception :
        return None 

def _download_with_progress (url :str ,dest :str ,line_callback =None )->bool :
    try :
        req =urllib .request .Request (url ,headers ={"User-Agent":"Revolt-App"})
        with urllib .request .urlopen (req ,timeout =30 )as resp :
            ...
            with open (dest ,"wb")as f :
                while True :
                    ...
                    f .write (chunk )
                    got +=len (chunk )
                    if line_callback and total :
                        pct =int (got *100 /total )
                        if pct !=last_pct and pct %5 ==0 :
                            last_pct =pct 
                            line_callback (f"Downloading… {pct }%  "
                            f"({got //1024 //1024 } MB / {total //1024 //1024 } MB)")
        return True 
    except Exception as e :
        if line_callback :
            line_callback (f"Download failed: {e }")
        return False 

def _run_streamed (cmd ,line_callback =None ,timeout =None ,cwd =None )->int :
    if line_callback :
        try :
            line_callback ("$ "+" ".join (str (c )for c in cmd ))
        except Exception :
            pass 
    try :
        proc =subprocess .Popen (cmd ,stdout =subprocess .PIPE ,stderr =subprocess .STDOUT ,
        ...
        if line_callback :
            line_callback (f"Couldn't run {cmd [0 ]}: {e }")
        return -1 
    try :
...
def _finish_install_with_defs (cb ,success_msg :str )->tuple [bool ,str ]:
    cb ("Fetching virus definitions…")
    ok ,msg =run_freshclam_update (cb )
    db_dir =resolve_clamav_db_dir ()
    if _db_dir_has_signatures (db_dir ):
        return True ,success_msg 
    return False ,(
    "ClamAV itself installed, but the virus definitions couldn't be "
    "downloaded automatically "
    f"({msg }). This is usually a network/firewall issue — if Revolt's "
    "Shield is on, try turning it off and clicking \"Update Definitions\" "
    "in this window again.")

def _install_clamav_windows (cb )->tuple [bool ,str ]:
    cb ("Looking up the latest official ClamAV release…")
    rel =_gh_latest_release (_CLAMAV_GH_REPO )
    asset_url =None 
    if rel :
        ...
        if not asset_url :
            for a in assets :
                if (a .get ("name")or "").lower ().endswith (".msi"):
                    asset_url =a .get ("browser_download_url");break 
    if not asset_url :
        return False ,("Couldn't find a Windows installer in the latest "
        "ClamAV release. Try the manual download instead.")
    cb (f"Found installer: {asset_url .rsplit ('/',1 )[-1 ]}")
    tmp_dir =tempfile .mkdtemp (prefix ="revolt_clamav_")
    msi_path =os .path .join (tmp_dir ,"clamav_setup.msi")
    cb ("Downloading…")
    if not _download_with_progress (asset_url ,msi_path ,cb ):
        return False ,"Download failed."
    cb ("Installing silently (this can take a minute)…")
    rc =_run_streamed (["msiexec","/i",msi_path ,"/qn","/norestart"],cb ,timeout =600 )
    if rc not in (0 ,3010 ):
        return False ,f"The installer exited with code {rc }."
    cb ("Install finished. Locating clamscan…")
    time .sleep (2 )
    if not find_clamscan ():
        return False ,"clamscan wasn't found after installation."
    return _finish_install_with_defs (cb ,"ClamAV installed successfully.")

def _install_clamav_macos (cb )->tuple [bool ,str ]:
    if shutil .which ("brew"):
        cb ("Installing via Homebrew…")
        rc =_run_streamed (["brew","install","clamav"],cb ,timeout =900 )
        if rc ==0 and find_clamscan ():
            return _finish_install_with_defs (cb ,"ClamAV installed successfully via Homebrew.")
        return False ,"Homebrew install didn't succeed — see the log above."
    return False ,("Homebrew wasn't found, so an automatic install isn't "
    "possible here. Install Homebrew from brew.sh, or use the manual "
    "download instead.")

_LINUX_PKG_MANAGERS =[
("apt-get",["apt-get","install","-y","clamav","clamav-daemon","clamav-freshclam"],["apt-get","update"]),
("dnf",["dnf","install","-y","clamav","clamav-update"],None ),
("yum",["yum","install","-y","clamav","clamav-update"],None ),
("pacman",["pacman","-Sy","--noconfirm","clamav"],None ),
("zypper",["zypper","--non-interactive","install","clamav"],None ),
]

def _install_clamav_from_source (cb )->tuple [bool ,str ]:
    cb ("No supported package manager found — building ClamAV from source instead.")
    if shutil .which ("apt-get"):
        deps =["build-essential","cmake","pkg-config","libssl-dev",
        "libcurl4-openssl-dev","zlib1g-dev","libbz2-dev",
        "libpcre2-dev","libjson-c-dev","check"]
        cb ("Installing build dependencies (apt)…")
        _run_streamed (_priv_wrap (["apt-get","update"]),cb ,timeout =300 )
        _run_streamed (_priv_wrap (["apt-get","install","-y"]+deps ),cb ,timeout =600 )
    elif shutil .which ("dnf")or shutil .which ("yum"):
        mgr ="dnf"if shutil .which ("dnf")else "yum"
        deps =["gcc","gcc-c++","cmake","pkgconfig","openssl-devel",
        "libcurl-devel","zlib-devel","bzip2-devel","pcre2-devel",
        "json-c-devel","check-devel"]
        cb (f"Installing build dependencies ({mgr })…")
        _run_streamed (_priv_wrap ([mgr ,"install","-y"]+deps ),cb ,timeout =600 )
    else :
        cb ("No known package manager found for build dependencies — "
        "trying anyway in case a compiler is already present.")

    if not (shutil .which ("cmake")and (shutil .which ("gcc")or shutil .which ("cc"))):
        return False ,("A compiler and cmake are required to build ClamAV "
        "from source and couldn't be installed automatically. Please "
        "install ClamAV manually from clamav.net.")

    cb ("Looking up the latest ClamAV source release…")
    rel =_gh_latest_release (_CLAMAV_GH_REPO )
    tarball_url =rel .get ("tarball_url")if rel else None 
    if not tarball_url :
        return False ,"Couldn't find the ClamAV source release."
    tmp_dir =tempfile .mkdtemp (prefix ="revolt_clamav_src_")
    tar_path =os .path .join (tmp_dir ,"clamav_src.tar.gz")
    cb ("Downloading source tarball…")
    if not _download_with_progress (tarball_url ,tar_path ,cb ):
        return False ,"Source download failed."
    cb ("Extracting…")
    try :
        import tarfile 
        ...
        return False ,f"Couldn't extract source: {e }"
    entries =[d for d in os .listdir (tmp_dir )
    if os .path .isdir (os .path .join (tmp_dir ,d ))]
    src_dirs =[d for d in entries if "clamav"in d .lower ()]or entries 
    if not src_dirs :
        return False ,"Couldn't locate the extracted source directory."
    src_dir =os .path .join (tmp_dir ,src_dirs [0 ])
    build_dir =os .path .join (src_dir ,"build")
    os .makedirs (build_dir ,exist_ok =True )

    cb ("Configuring build (cmake)…")
    rc =_run_streamed (["cmake",".."],cb ,timeout =300 ,cwd =build_dir )
    if rc !=0 :
        return False ,"cmake configuration failed — see the log above."
    cb ("Compiling ClamAV — this can take several minutes…")
    rc =_run_streamed (["make",f"-j{os .cpu_count ()or 2 }"],cb ,timeout =3600 ,cwd =build_dir )
    if rc !=0 :
        return False ,"Compilation failed — see the log above."
    cb ("Installing compiled binaries…")
    rc =_run_streamed (_priv_wrap (["make","install"]),cb ,timeout =300 ,cwd =build_dir )
    if rc !=0 :
        return False ,"'make install' failed (root/sudo privileges are required)."
    time .sleep (1 )
    if not find_clamscan ():
        return False ,("Build finished but clamscan wasn't found afterwards — "
        "check that /usr/local/bin is on PATH.")
    return _finish_install_with_defs (cb ,"ClamAV compiled and installed successfully.")

def _install_clamav_linux (cb )->tuple [bool ,str ]:
    for name ,install_cmd ,update_cmd in _LINUX_PKG_MANAGERS :
        if not shutil .which (name ):
            continue 
        cb (f"Detected package manager: {name }")
        if update_cmd :
            _run_streamed (_priv_wrap (update_cmd ),cb ,timeout =300 )
        rc =_run_streamed (_priv_wrap (install_cmd ),cb ,timeout =600 )
        if rc ==0 :
            time .sleep (1 )
            if find_clamscan ():
                return _finish_install_with_defs (cb ,f"ClamAV installed successfully via {name }.")
        cb (f"{name } install didn't succeed (exit {rc }).")
        break 
    return _install_clamav_from_source (cb )

def install_clamav (line_callback =None ,stop_event =None )->tuple [bool ,str ]:
    ...
    try :
        if sys .platform .startswith ("win"):
            return _install_clamav_windows (cb )
        elif sys .platform =="darwin":
            return _install_clamav_macos (cb )
        else :
            return _install_clamav_linux (cb )
    except _InstallCancelled :
        return False ,"Installation cancelled."
    except Exception as e :
        return False ,f"Unexpected error during install: {e }"
...
_OPENVPN_DOWNLOADS_PAGE ="https://openvpn.net/community-downloads/"

def _fetch_latest_openvpn_msi_url ()->str |None :
    try :
        req =urllib .request .Request (_OPENVPN_DOWNLOADS_PAGE ,
        headers ={"User-Agent":"Revolt-App"})
        with urllib .request .urlopen (req ,timeout =20 )as resp :
            html =resp .read ().decode ("utf-8","ignore")
    except Exception :
        return None 
    urls =re .findall (
    r"https://swupdate\.openvpn\.org/community/releases/OpenVPN-[\w.\-]+?\.msi",
    html ,re .IGNORECASE )
    if not urls :
        return None 
    urls =sorted (set (urls ))
    amd64 =[u for u in urls if "amd64"in u .lower ()]
    pool =amd64 or urls 
    def ver_key (u ):
        m =re .search (r"OpenVPN-(\d+)\.(\d+)\.(\d+)",u )
        return tuple (int (x )for x in m .groups ())if m else (0 ,0 ,0 )
    pool .sort (key =ver_key ,reverse =True )
    return pool [0 ]

def _install_openvpn_windows (cb )->tuple [bool ,str ]:
    cb ("Looking up the latest official OpenVPN release…")
    msi_url =_fetch_latest_openvpn_msi_url ()
    if not msi_url :
        return False ,("Couldn't find a Windows installer on openvpn.net. "
        "Try the manual download instead.")
    cb (f"Found installer: {msi_url .rsplit ('/',1 )[-1 ]}")
    tmp_dir =tempfile .mkdtemp (prefix ="revolt_openvpn_")
    msi_path =os .path .join (tmp_dir ,"openvpn_setup.msi")
    cb ("Downloading…")
    if not _download_with_progress (msi_url ,msi_path ,cb ):
        return False ,"Download failed."
    cb ("Installing silently (this can take a minute)…")
    rc =_run_streamed (["msiexec","/i",msi_path ,"/qn","/norestart"],cb ,timeout =600 )
    if rc not in (0 ,3010 ):
        return False ,(f"The installer exited with code {rc }. Windows may "
        "have blocked the elevation prompt — try running Revolt as "
        "Administrator and installing again.")
    cb ("Install finished. Locating openvpn.exe…")
    time .sleep (2 )
    if not _detect_vpn_tool ("openvpn"):
        return False ,"openvpn.exe wasn't found after installation."
    return True ,"OpenVPN installed successfully."

def _install_openvpn_macos (cb )->tuple [bool ,str ]:
    if shutil .which ("brew"):
        cb ("Installing via Homebrew…")
        rc =_run_streamed (["brew","install","openvpn"],cb ,timeout =900 )
        if rc ==0 and _detect_vpn_tool ("openvpn"):
            return True ,"OpenVPN installed successfully via Homebrew."
        return False ,"Homebrew install didn't succeed — see the log above."
    return False ,("Homebrew wasn't found, so an automatic install isn't "
    "possible here. Install Homebrew from brew.sh, or use the manual "
    "download instead.")

_OPENVPN_LINUX_PKG_MANAGERS =[
("apt-get",["apt-get","install","-y","openvpn"],["apt-get","update"]),
("dnf",["dnf","install","-y","openvpn"],None ),
("yum",["yum","install","-y","openvpn"],None ),
("pacman",["pacman","-Sy","--noconfirm","openvpn"],None ),
("zypper",["zypper","--non-interactive","install","openvpn"],None ),
]

_OPENVPN_SRC_TARBALL ="https://swupdate.openvpn.org/community/releases/openvpn-2.6.14.tar.gz"

def _install_openvpn_from_source (cb )->tuple [bool ,str ]:
    cb ("No supported package manager found — building OpenVPN from source instead.")
    if shutil .which ("apt-get"):
        deps =["build-essential","autoconf","automake","libtool","pkg-config",
        "libssl-dev","liblzo2-dev","libpam0g-dev","libnl-genl-3-dev","linux-libc-dev"]
        cb ("Installing build dependencies (apt)…")
        _run_streamed (_priv_wrap (["apt-get","update"]),cb ,timeout =300 )
        _run_streamed (_priv_wrap (["apt-get","install","-y"]+deps ),cb ,timeout =600 )
    elif shutil .which ("dnf")or shutil .which ("yum"):
        mgr ="dnf"if shutil .which ("dnf")else "yum"
        deps =["gcc","make","autoconf","automake","libtool","pkgconfig",
        "openssl-devel","lzo-devel","pam-devel"]
        cb (f"Installing build dependencies ({mgr })…")
        _run_streamed (_priv_wrap ([mgr ,"install","-y"]+deps ),cb ,timeout =600 )
    else :
        cb ("No known package manager found for build dependencies — "
    ...
    if not (shutil .which ("gcc")or shutil .which ("cc")):
        return False ,("A C compiler is required to build OpenVPN from "
        "source and couldn't be installed automatically. Please install "
        "OpenVPN manually from openvpn.net.")

    cb ("Downloading OpenVPN source tarball…")
    tmp_dir =tempfile .mkdtemp (prefix ="revolt_openvpn_src_")
    tar_path =os .path .join (tmp_dir ,"openvpn_src.tar.gz")
    if not _download_with_progress (_OPENVPN_SRC_TARBALL ,tar_path ,cb ):
        return False ,"Source download failed."
    cb ("Extracting…")
    try :
        import tarfile 
    ...
    entries =[d for d in os .listdir (tmp_dir )
    if os .path .isdir (os .path .join (tmp_dir ,d ))]
    src_dirs =[d for d in entries if "openvpn"in d .lower ()]or entries 
    if not src_dirs :
        return False ,"Couldn't locate the extracted source directory."
    src_dir =os .path .join (tmp_dir ,src_dirs [0 ])

    cb ("Configuring build (./configure)…")
    rc =_run_streamed (["./configure"],cb ,timeout =300 ,cwd =src_dir )
    if rc !=0 :
        return False ,"./configure failed — see the log above."
    cb ("Compiling OpenVPN — this can take several minutes…")
    rc =_run_streamed (["make",f"-j{os .cpu_count ()or 2 }"],cb ,timeout =3600 ,cwd =src_dir )
    if rc !=0 :
        return False ,"Compilation failed — see the log above."
    cb ("Installing compiled binary…")
    rc =_run_streamed (_priv_wrap (["make","install"]),cb ,timeout =300 ,cwd =src_dir )
    if rc !=0 :
        return False ,"'make install' failed (root/sudo privileges are required)."
    time .sleep (1 )
    if not _detect_vpn_tool ("openvpn"):
        return False ,("Build finished but openvpn wasn't found afterwards — "
        "check that /usr/local/sbin is on PATH.")
    return True ,"OpenVPN compiled and installed successfully."

def _install_openvpn_linux (cb )->tuple [bool ,str ]:
    for name ,install_cmd ,update_cmd in _OPENVPN_LINUX_PKG_MANAGERS :
        if not shutil .which (name ):
            continue 
        cb (f"Detected package manager: {name }")
        if update_cmd :
        ...
        if rc ==0 :
            time .sleep (1 )
            if _detect_vpn_tool ("openvpn"):
                return True ,f"OpenVPN installed successfully via {name }."
        cb (f"{name } install didn't succeed (exit {rc }).")
        break 
    return _install_openvpn_from_source (cb )

def install_openvpn (line_callback =None ,stop_event =None )->tuple [bool ,str ]:
    ...
    try :
        if sys .platform .startswith ("win"):
            return _install_openvpn_windows (cb )
        elif sys .platform =="darwin":
            return _install_openvpn_macos (cb )
        else :
            return _install_openvpn_linux (cb )
    except _InstallCancelled :
        return False ,"Installation cancelled."
    except Exception as e :
        return False ,f"Unexpected error during install: {e }"
...
def _windows_active_vpn_interfaces ()->list [str ]:
    ...
    for block in blocks :
        ...
        if not any (m in block_low for m in _VPN_IFACE_MARKERS ):
            continue 
...
def _openvpn_processes_running ()->bool :
    try :
        if os .name =="nt":
            r =subprocess .run (["tasklist","/FI","IMAGENAME eq openvpn.exe"],
            capture_output =True ,text =True ,startupinfo =_si_hidden (),timeout =8 )
            return "openvpn.exe"in r .stdout .lower ()
        else :
            r =subprocess .run (["pgrep","-x","openvpn"],capture_output =True ,timeout =8 )
            return r .returncode ==0 
    except Exception :
        return False 

def _kill_all_vpn_named_tasks ()->list [str ]:
    killed =[]
    ...
    try :
        if os .name =="nt":
            ...
            r =subprocess .run (["ps","-eo","comm"],capture_output =True ,text =True ,timeout =8 )
            names =set ()
...
def _list_vpn_named_tasks ()->list [dict ]:
    tasks =[]
    our_exe =os .path .basename (sys .executable ).lower ()
    our_pid =os .getpid ()
    try :
        if os .name =="nt":
            ...
            for line in r .stdout .splitlines ():
                ...
                try :
                    ...
                    continue 
                if "vpn"not in name .lower ()or name .lower ()==our_exe or pid ==our_pid :
                    continue 
                tasks .append ({"name":name ,"pid":pid })
        else :
    ...
    for t in tasks :
        ...
        seen .add (key )
        unique .append (t )
    unique .sort (key =lambda t :(t ["name"].lower (),t ["pid"]))
    return unique 

def _end_task_by_pid (pid :int )->tuple [bool ,str ]:
    try :
        if os .name =="nt":
            ...
            return False ,(r .stderr or r .stdout or "Failed to end the task.").strip ()
        else :
            os .kill (pid ,9 )
            return True ,"Ended."
    ...
    try :
        if kind =="wireguard":
            ...
            if os .name =="nt"and tool :
                r =subprocess .run ([tool ,"/uninstalltunnelservice",tunnel_name ],
                capture_output =True ,text =True ,startupinfo =_si_hidden (),timeout =20 )
                if r .returncode !=0 and _iface_has_address (tunnel_name ):
                    return False ,(r .stderr or r .stdout or 
                    "Failed to remove the tunnel service — it's still up.").strip ()
            else :
                r =subprocess .run (["wg-quick","down",config_path ],
                capture_output =True ,text =True ,timeout =20 )
                stderr =(r .stderr or "").lower ()
                if r .returncode !=0 and "is not a wireguard interface"not in stderr :
                    return False ,(r .stderr or "Failed to bring the tunnel down.").strip ()

            down =_wait_for (lambda :not _iface_has_address (tunnel_name ),timeout =10 )
            if not down :
                return False ,("Told the tunnel to stop, but the network "
                "interface still has an address — it may need a moment, "
                "or you may need to remove it manually.")
        else :
            pid =state .get ("pid")
            if os .name =="nt":
                if pid :
                    subprocess .run (["taskkill","/PID",str (pid ),"/F"],
                    capture_output =True ,startupinfo =_si_hidden (),timeout =8 )
                if _openvpn_processes_running ():
                    subprocess .run (["taskkill","/IM","openvpn.exe","/F"],
                    capture_output =True ,startupinfo =_si_hidden (),timeout =8 )
            else :
                target_pid =pid 
                if not target_pid and os .path .exists (VPN_PID_FILE ):
                    try :
                        with open (VPN_PID_FILE )as f :
                        ...
                        target_pid =None 
                if target_pid :
                    try :os .kill (target_pid ,15 )
                    except ProcessLookupError :pass 
                if _openvpn_processes_running ():
                    subprocess .run (["pkill","-9","-x","openvpn"],
                    capture_output =True ,timeout =8 )

            gone =_wait_for (lambda :not _openvpn_processes_running (),timeout =8 ,interval =0.3 )
            if not gone :
                return False ,("Sent the stop signal, but an OpenVPN process "
                "is still running somewhere on this system — it may be "
                "elevated with different permissions than Revolt, or "
                "another app is managing it. Try closing it manually "
                "(Task Manager → openvpn.exe).")

            lingering =any_vpn_interface_active ()
            for name in lingering :
                force_down_interface (name )
            if lingering :
                torn_down =_wait_for (lambda :not any_vpn_interface_active (),timeout =8 ,interval =0.3 )
                if not torn_down :
                    return False ,("OpenVPN's process stopped, but its network "
                    "adapter is still up and routing traffic (likely a "
                    "\"persist-tun\" setting in the config) — try disabling "
                    "it manually from Network Connections if this keeps "
                    "happening.")

        _kill_all_vpn_named_tasks ()

        if os .path .exists (VPN_PID_FILE ):
            try :os .remove (VPN_PID_FILE )
            except Exception :pass 
        _clear_vpn_state ()
        return True ,"Disconnected — verified the tunnel is actually down."
    except Exception as e :
        return False ,str (e )

def fetch_vpn_location (timeout :float =6.0 )->dict |None :
    endpoints =(
    ("https://ipapi.co/json/","ipapi"),
    ("http://ip-api.com/json/?fields=status,message,country,city,query","ip-api"),
    )
    for url ,fmt in endpoints :
        try :
            ...
            if not (city or country ):
                continue 
            return {"city":city ,"country":country ,"ip":ip }
        except Exception :
            continue 
    return None 

HOTSPOT_CONFIG_FILE =os .path .join (APP_DATA_DIR ,"revolt_hotspot_config.json")
...
VPNGATE_CACHE_DIR =os .path .join (APP_DATA_DIR ,"vpngate_cache")
...
def load_vpn_rotate_config ()->dict :
    if os .path .exists (VPN_ROTATE_CONFIG_FILE ):
        try :
            with open (VPN_ROTATE_CONFIG_FILE )as f :
                data =json .load (f )
            if isinstance (data ,dict ):
                return data 
        except Exception :
...
def save_vpn_rotate_config (data :dict ):
    _atomic_write_json (VPN_ROTATE_CONFIG_FILE ,data )

def _pick_random_vpngate_relay (relays :list [dict ],exclude_ip :str |None =None )->dict |None :
    pool =[r for r in relays if r .get ("config_b64")and r .get ("ip")!=exclude_ip ]
    if not pool :
        pool =[r for r in relays if r .get ("config_b64")]
    if not pool :
        return None 
    pool =sorted (pool ,key =lambda r :r .get ("score",0 ),reverse =True )
    top_n =max (5 ,len (pool )//4 )
    return secrets .choice (pool [:top_n ])
...
class SidebarBtn (tk .Canvas ):
    ...
    icon_picker =None ,icon =None ,icon_color =None ,icon_fn =None ,icon_image =None ,**kw ):
    ...
    def _draw (self ):
        ...
        if self ._icon_image is not None or self ._icon_fn or self ._icon :
            icon_col_x =24 
            label_x =icon_col_x +20 
            icon_color =self ._icon_color or text_col 
            if self ._icon_image is not None :
                self .create_image (icon_col_x ,h //2 ,image =self ._icon_image ,anchor ="center")
            elif self ._icon_fn :
...
class Toast (tk .Toplevel ):
    ...
    count_to :int =None ,kind :str =None ,duration_ms :int =3800 ):
        ...
        self._after_id_count = None
        self._after_id_fade = None
        if count_to is not None :
            self .msg_lbl .config (text =msg .format (count =0 ))
            self._after_id_count = self .after (120 ,lambda :self ._count_up (count_to ,time .perf_counter ()))
        self._after_id_fade = self .after (duration_ms ,self ._start_fade_out )
    ...
    def _on_closed (self ):
        ...
        if self._after_id_count is not None:
            try:
                self.after_cancel(self._after_id_count)
            except Exception:
                pass
        if self._after_id_fade is not None:
            try:
...
class Revolt (tk .Tk ):
    ...
    def __init__ (self ):
        ...
        self ._vpn_rotate_active =False 
        self ._vpn_rotate_interval_min =30 
        self ._vpn_rotate_next_at =0.0 
        self ._vpn_rotate_after_id =None 
        self ._vpn_rotate_state ={"current_ip":None }
        ...
        self .after (1500 ,self ._resume_vpn_rotation_if_needed )
    ...
    def _minimize_to_tray (self ):
        ...
        threading .Thread (
            ...
            args =("Revolt","Revolt is still running in the system tray. "
    ...
    def _apply_theme_data (self ):
        ...
        ACCENT =self .theme_data .get ("accent","#606060")
    ...
    def _open_settings (self ):
        ...
        def build_security_tools (parent ,want ):
            scan_title ="Virus / Malware Scanner"
            scan_desc =("Scan any file, folder, .zip, .rar or .7z with the "
            "open-source ClamAV engine."if clamav_available ()else 
            "Install the free, open-source ClamAV engine to enable scanning.")
            breach_title ="Breach Check"
            breach_desc =("Check a password, email, or website domain against "
            "known data breaches (HaveIBeenPwned).")
            rows =[]
            if want (scan_title ,scan_desc ):
                rows .append (("🧪",scan_title ,scan_desc ,self ._open_virus_scanner ))
            if want (breach_title ,breach_desc ):
                rows .append (("🔓",breach_title ,breach_desc ,self ._open_breach_check ))
            if not rows :
                return 
            _ ,card =self ._settings_group (parent ,"Security Tools")
            for icon ,title ,desc ,cmd in rows :
                self ._settings_row (card ,icon ,title ,desc ,cmd ,accent =ACCENT2 )
        ...
        ("security","🧪","Security Tools",build_security_tools ),
    ...
    def _build (self ):
        ...
        def _set_hdr_icon (topic =None ):
            ...
            self ._hdr_icon_photo =None 
            has_custom =bool (topic )and topic in self .custom_icons and self .custom_icons .get (topic )!=self .ICONS .get (topic )
            custom_val =self .custom_icons .get (topic )if has_custom else None 
            if has_custom and is_custom_image_icon (custom_val ):
                photo =render_custom_icon_photo (resolve_custom_icon_path (custom_val ),30 )
                if photo is not None :
                    self ._hdr_icon_photo =photo 
                    self .hdr_icon .create_image (18 ,24 ,image =photo ,anchor ="center")
                    return 
                has_custom =False 
            ...
            if fn :
                ...
                glyph =self .ICONS .get (topic ,"◆")if topic else "◆"
                if has_custom and not is_custom_image_icon (custom_val ):
                    glyph =custom_val 
        ...
        self .vpn_location_lbl =tk .Label (top_foot ,text ="",
        font =(FONT_FAMILY ,12 ,"bold"),fg ="#3ddc97",bg =BG )
        self .vpn_location_lbl .pack (side ="left",padx =(10 ,0 ))
        self ._vpn_location_cache ={"ip":None }
        ...
        self ._version_lbl =tk .Label (self ,text ="v1.07",
    ...
    def _build_sidebar_buttons (self ):
        ...
        for topic in self .topic_order :
            ...
            icon_image =None 
            if has_custom and is_custom_image_icon (icon ):
                icon_image =render_custom_icon_photo (resolve_custom_icon_path (icon ),22 )
                if icon_image is None :
                    icon =self .ICONS .get (topic ,"◆")
                    has_custom =False 
            ...
            icon =icon if icon_image is None else None ,
            icon_fn =icon_fn ,icon_color =icon_color ,icon_image =icon_image )
            btn .pack (fill ="x",pady =2 ,padx =10 )
    ...
    def _open_icon_picker (self ,topic :str ):
        ...
        def _apply (value :str ):
            old =self .custom_icons .get (topic )
            if old and is_custom_image_icon (old )and old !=value :

                delete_custom_icon_file (old )
            self .custom_icons [topic ]=value 
        ...
        for i ,icon in enumerate (self ._ICON_CHOICES ):
            ...
            is_current =_current_value ()==icon 
        ...
        custom_row .pack (fill ="x",padx =20 ,pady =(6 ,14 ))
        ...
        sep .pack (fill ="x",padx =20 ,pady =(2 ,12 ))

        upload_row =tk .Frame (win ,bg =BG )
        upload_row .pack (fill ="x",padx =20 ,pady =(0 ,18 ))
        tk .Label (upload_row ,text ="Or upload your own image:",
        font =(FONT_FAMILY ,9 ),fg =MUTED ,bg =BG ).pack (anchor ="w")

        preview_wrap =tk .Frame (upload_row ,bg =BG )
        preview_wrap .pack (fill ="x",pady =(8 ,0 ))

        preview_cv =tk .Canvas (preview_wrap ,width =40 ,height =40 ,bg =BG ,
        highlightthickness =1 ,highlightbackground =BORDER )
        preview_cv .pack (side ="left")

        btn_col =tk .Frame (preview_wrap ,bg =BG )
        btn_col .pack (side ="left",padx =(12 ,0 ),fill ="x",expand =True )

        status_lbl =tk .Label (btn_col ,text ="",font =(FONT_FAMILY ,9 ),
        fg =MUTED ,bg =BG ,anchor ="w",justify ="left",wraplength =260 )
        status_lbl .pack (anchor ="w",pady =(0 ,6 ))

        btn_row =tk .Frame (btn_col ,bg =BG )
        btn_row .pack (anchor ="w")

        def _refresh_preview ():
            preview_cv .delete ("all")
            val =_current_value ()
            if is_custom_image_icon (val ):
                photo =render_custom_icon_photo (resolve_custom_icon_path (val ),34 )
                if photo is not None :
                    preview_cv ._photo_ref =photo 
                    preview_cv .create_image (20 ,20 ,image =photo ,anchor ="center")
                    status_lbl .config (text ="Using a custom uploaded image.")
                    remove_btn .pack (side ="left",padx =(8 ,0 ))
                    return 
            key ,_rest =_split_icon_text (val )
            if key and _rest =="":
                draw_icon_glyph (preview_cv ,20 ,20 ,key ,18 ,TEXT ,bg =BG )
            else :
                preview_cv .create_text (20 ,20 ,text =val ,font =(FONT_FAMILY ,16 ))
            status_lbl .config (text ="Using a built-in icon."
            if not self .custom_icons .get (topic )else "Using a custom character.")
            remove_btn .pack_forget ()

        def _do_upload ():
            filetypes =[("Images","*.png *.jpg *.jpeg *.gif *.bmp *.ico *.webp"+
            (" *.svg"if _SVG_ICONS_AVAILABLE else "")),
            ("All files","*.*")]
            path =filedialog .askopenfilename (title =f"Choose an image icon for “{topic }”",
            filetypes =filetypes )
            if not path :
                return 
            ref =import_custom_topic_icon (path )
            if not ref :
                messagebox .showerror ("Couldn't use that image",
                "That file couldn't be used as an icon. Try a PNG, JPG, "
                "GIF, BMP, ICO or WEBP under 8 MB"+
                (", or SVG"if _SVG_ICONS_AVAILABLE else "")+".",
                parent =win )
                return 
            _apply (ref )
            _refresh_preview ()
        ...
        def _do_remove ():
            old =self .custom_icons .get (topic )
            if old :
                if is_custom_image_icon (old ):
                    delete_custom_icon_file (old )
                self .custom_icons .pop (topic ,None )
                self .theme_data ["icons"]=self .custom_icons 
                self ._save_config_and_sync ()
                if topic in self ._sb_btns :
                    self ._build_sidebar_buttons ()
        ...
        upload_label ="Upload image…"if _PIL_AVAILABLE else "Upload image (needs Pillow)"
        upload_btn =FlatBtn (btn_row ,text =upload_label ,
        base_bg =SIDEBAR ,hover_bg =lighten (ACCENT_DIM ,.06 ),
        fg =TEXT ,font =(FONT_FAMILY ,10 ,"bold"),
        command =_do_upload ,border_col =lighten (BORDER ,.35 ),
        ghost =True ,radius =16 ,height =32 ,width =130 ,
        disabled =not _PIL_AVAILABLE )
        upload_btn .pack (side ="left")

        remove_btn =FlatBtn (btn_row ,text ="Remove",
        base_bg =SIDEBAR ,hover_bg =lighten (DANGER ,.1 ),
        fg =DANGER ,font =(FONT_FAMILY ,10 ,"bold"),
        command =_do_remove ,border_col =DANGER ,
        ghost =True ,radius =16 ,height =32 ,width =74 )
    ...
    def _delete_topic (self ):
        ...
        f'Delete "{self .selected }"?',parent =self ):
            ...
            _old_icon =self .custom_icons .pop (self .selected ,None )
            if _old_icon and is_custom_image_icon (_old_icon ):
                delete_custom_icon_file (_old_icon )
            self .theme_data ["order"]=self .topic_order 
            self .theme_data ["icons"]=self .custom_icons 
            self .selected =next (iter (self .topics ),None )
            self ._save_config_and_sync ()
    ...
    def _open_virus_scanner (self ):
        win =tk .Toplevel (self )
        win .title ("Virus & Malware Scanner")
        win .geometry ("560x660")
        win .minsize (480 ,540 )
        win .configure (bg =BG )
        win .transient (self )
        win .resizable (True ,True )
        apply_titlebar_theme (win ,self .theme_data .get ("titlebar_color"))

        body =tk .Frame (win ,bg =BG ,padx =22 ,pady =18 )
        body .pack (fill ="both",expand =True )

        tk .Label (body ,text ="🧪  Virus & Malware Scanner",
        font =(FONT_FAMILY ,16 ,"bold"),fg =TEXT ,bg =BG ,anchor ="w"
        ).pack (anchor ="w")
        tk .Label (body ,
        text ="Scans a file, folder, or archive (.zip, .rar, .7z) using "
        "ClamAV — a free, open-source antivirus engine trusted "
        "worldwide. Nothing is uploaded anywhere; the scan runs "
        "entirely on this machine.",
        font =(FONT_FAMILY ,9 ),fg =MUTED ,bg =BG ,anchor ="w",
        wraplength =500 ,justify ="left").pack (anchor ="w",pady =(2 ,14 ))

        state ={"scanning":False ,"installing":False ,"stop_evt":None ,"tool":find_clamscan ()}

        notice =tk .Frame (body ,bg =CARD ,highlightthickness =1 ,
        highlightbackground =WARN )
        notice_inner =tk .Frame (notice ,bg =CARD ,padx =14 ,pady =10 )
        notice_inner .pack (fill ="x")
        notice_lbl =tk .Label (notice_inner ,
        text ="⚠  ClamAV isn't installed on this system yet. Click "
        "Install ClamAV to fetch and set it up automatically — no "
        "browser or manual steps needed.",
        font =(FONT_FAMILY ,9 ,"bold"),fg =WARN ,bg =CARD ,anchor ="w",
        wraplength =460 ,justify ="left")
        install_holder ={"fn":lambda :None ,"btn":None }

        notice_lbl .pack (anchor ="w")
        notice_btn_row =tk .Frame (notice_inner ,bg =CARD )
        notice_btn_row .pack (anchor ="w",pady =(8 ,0 ))
        install_holder ["btn"]=FlatBtn (notice_btn_row ,text ="⚙  Install ClamAV",
        base_bg =CARD ,hover_bg =lighten (CARD ,.08 ),fg =WARN ,ghost =True ,
        border_col =WARN ,command =lambda :install_holder ["fn"](),
        radius =12 ,height =30 ,width =150 ,font =(FONT_FAMILY ,9 ,"bold"))
        install_holder ["btn"].pack (side ="left",padx =(0 ,8 ))

        FlatBtn (notice_btn_row ,text ="Manual Download",base_bg =CARD ,
        hover_bg =lighten (CARD ,.08 ),fg =MUTED ,ghost =True ,
        border_col =lighten (BORDER ,.2 ),
        command =lambda :webbrowser .open ("https://www.clamav.net/downloads"),
        radius =12 ,height =30 ,width =128 ,font =(FONT_FAMILY ,9 ,"bold")
        ).pack (side ="left",padx =(0 ,8 ))

        def recheck ():
            state ["tool"]=find_clamscan ()
            refresh_availability ()
        FlatBtn (notice_btn_row ,text ="🔄  Recheck",base_bg =BORDER ,fg =TEXT ,
        command =recheck ,radius =12 ,height =30 ,width =110 ,
        font =(FONT_FAMILY ,9 ,"bold")).pack (side ="left")

        picker =tk .Frame (body ,bg =BG )
        path_var =tk .StringVar (value ="")
        path_entry =tk .Entry (picker ,textvariable =path_var ,
        font =(FONT_FAMILY ,10 ),bg =CARD ,fg =TEXT ,
        insertbackground =TEXT ,relief ="flat")
        path_entry .pack (side ="left",fill ="x",expand =True ,ipady =6 ,padx =(0 ,8 ))

        def browse_file ():
            p =filedialog .askopenfilename (
            title ="Choose a file or archive to scan",
            filetypes =[("All files","*.*"),
        ...
        FlatBtn (picker ,text ="📄 File",base_bg =BORDER ,fg =TEXT ,
        command =browse_file ,radius =10 ,height =32 ,width =78 ,
        font =(FONT_FAMILY ,9 ,"bold")).pack (side ="left",padx =(0 ,6 ))
        FlatBtn (picker ,text ="📁 Folder",base_bg =BORDER ,fg =TEXT ,
        command =browse_folder ,radius =10 ,height =32 ,width =88 ,
        font =(FONT_FAMILY ,9 ,"bold")).pack (side ="left")

        action_row =tk .Frame (body ,bg =BG )
        status_lbl =tk .Label (action_row ,text ="Ready.",
        font =(FONT_FAMILY ,9 ),fg =MUTED ,bg =BG ,anchor ="w")
        status_lbl .pack (side ="left",fill ="x",expand =True )

        scan_btn_holder ={"btn":None }

        log_frame =tk .Frame (body ,bg =CARD ,highlightthickness =1 ,
        highlightbackground =BORDER )
        log_text =tk .Text (log_frame ,bg =CARD ,fg =MUTED ,
        font =("Consolas"if os .name =="nt"else "Courier",9 ),
        relief ="flat",wrap ="none",height =14 ,state ="disabled")
        log_scroll =tk .Scrollbar (log_frame ,command =log_text .yview )
        log_text .configure (yscrollcommand =log_scroll .set )
        log_text .tag_config ("bad",foreground =DANGER )
        log_text .tag_config ("ok",foreground ="#3ddc97")

        summary_lbl =tk .Label (body ,text ="",font =(FONT_FAMILY ,11 ,"bold"),
        bg =BG ,anchor ="w",justify ="left",wraplength =500 )

        def append_log (line ,bad =False ):
        ...
        def do_scan ():
            path =path_var .get ().strip ().strip ('"')
            if not path :
                messagebox .showwarning ("No Path Selected",
                "Choose a file or folder to scan first.",parent =win )
                return 
            if not os .path .exists (path ):
                messagebox .showerror ("Not Found",
                "That file or folder doesn't exist.",parent =win )
                return 
            if not state ["tool"]:
                messagebox .showwarning ("ClamAV Not Installed",
                "Install ClamAV first, then click Recheck.",parent =win )
                return 

            state ["scanning"]=True 
            state ["stop_evt"]=threading .Event ()
            scan_btn_holder ["btn"].set_disabled (True )
            scan_btn_holder ["btn"].config (text ="Scanning…")
            status_lbl .config (text =f"Scanning “{os .path .basename (path )or path }”…",fg =MUTED )
            ...
            file_count ={"n":0 }
            def on_line (line ,is_hit ):
                if is_hit :
                    file_count ["n"]+=1 
                def ui ():
                    if win .winfo_exists ():
                        append_log (line ,bad =is_hit )
                        if is_hit :
                            status_lbl .config (
                            text =f"⚠ {file_count ['n']} threat(s) found so far…",fg =DANGER )
                self .after (0 ,ui )

            def worker ():
                result =run_clamav_scan (path ,line_callback =on_line ,
                stop_event =state ["stop_evt"])
                def done ():
                    state ["scanning"]=False 
                    if not win .winfo_exists ():
                        return 
                    scan_btn_holder ["btn"].set_disabled (False )
                    scan_btn_holder ["btn"].config (text ="▶  Start Scan")
                    if not result ["ok"]and result .get ("error")and not result ["threats"]:
                        status_lbl .config (text ="Scan couldn't start.",fg =DANGER )
                        summary_lbl .config (text =f"⚠️  {result ['error']}",fg =DANGER )
                        return 
                    threats =result ["threats"]
                    scanned =result .get ("scanned")or "?"
                    if threats :
                        status_lbl .config (text ="Scan complete.",fg =MUTED )
                        names =", ".join (sorted ({t [1 ]for t in threats }))[:200 ]
                        summary_lbl .config (
                        text =f"🚫  {len (threats )} threat(s) found in {scanned } file(s) scanned.\n"
                        f"Detected: {names }",fg =DANGER )
                        Toast (self ,"Threats Found 🚫",
                        f"{len (threats )} infected file(s) detected.",ok =False )
                    else :
                        status_lbl .config (text ="Scan complete.",fg =MUTED )
                        summary_lbl .config (
                        text =f"✅  Clean — no threats found in {scanned } file(s) scanned.",
                        fg ="#3ddc97")
                        Toast (self ,"Scan Complete ✅",
                        "No threats were found.",ok =True )
                self .after (0 ,done )
            threading .Thread (target =worker ,daemon =True ).start ()

        def do_stop ():
            if state ["stop_evt"]is not None :
                state ["stop_evt"].set ()
                status_lbl .config (text ="Stopping…",fg =MUTED )

        scan_btn_holder ["btn"]=FlatBtn (action_row ,text ="▶  Start Scan",
        base_bg =ACCENT2 ,fg ="#ffffff",command =do_scan ,
        radius =12 ,height =32 ,width =120 ,font =(FONT_FAMILY ,9 ,"bold"))

        def do_update_defs ():
            if not find_freshclam ():
                messagebox .showinfo ("Not Available",
                "freshclam wasn't found alongside your ClamAV install.",
                parent =win )
                return 
            status_lbl .config (text ="Updating virus definitions…",fg =MUTED )
            def worker ():
                ok ,msg =run_freshclam_update ()
                def done ():
                    if not win .winfo_exists ():
                        return 
                    status_lbl .config (text =msg if ok else "Update finished with a message — see log.",
                    fg ="#3ddc97"if ok else WARN )
                    if ok :
                        Toast (self ,"Definitions Updated ✅",msg ,ok =True )
                self .after (0 ,done )
            threading .Thread (target =worker ,daemon =True ).start ()

        def do_manage_custom_defs ():
            cwin =tk .Toplevel (win )
            cwin .title ("Custom Virus Definitions")
            cwin .geometry ("480x420")
            cwin .configure (bg =BG )
            cwin .transient (win )
            apply_titlebar_theme (cwin ,self .theme_data .get ("titlebar_color"))

            cbody =tk .Frame (cwin ,bg =BG ,padx =18 ,pady =16 )
            cbody .pack (fill ="both",expand =True )

            tk .Label (cbody ,text ="📑  Custom Definitions",
            font =(FONT_FAMILY ,13 ,"bold"),fg =TEXT ,bg =BG ,anchor ="w"
            ).pack (anchor ="w")
            tk .Label (cbody ,
            text ="Load your own ClamAV-format signature files (.hdb, "
            ".hsb, .ndb, .ldb) or YARA rules (.yar/.yara) — e.g. hash or "
            "pattern feeds from a threat-intel source. Revolt only "
            "loads real signature files in these formats; it does not "
            "write or invent malware definitions.",
            font =(FONT_FAMILY ,9 ),fg =MUTED ,bg =BG ,anchor ="w",
            wraplength =430 ,justify ="left").pack (anchor ="w",pady =(4 ,12 ))

            list_frame =tk .Frame (cbody ,bg =CARD ,highlightthickness =1 ,
            highlightbackground =BORDER )
            list_frame .pack (fill ="both",expand =True ,pady =(0 ,10 ))
            defs_list =tk .Listbox (list_frame ,bg =CARD ,fg =TEXT ,
            font =(FONT_FAMILY ,9 ),relief ="flat",highlightthickness =0 ,
            selectbackground =ACCENT2 ,activestyle ="none")
            defs_scroll =tk .Scrollbar (list_frame ,command =defs_list .yview )
            defs_list .configure (yscrollcommand =defs_scroll .set )
            defs_list .pack (side ="left",fill ="both",expand =True )
            defs_scroll .pack (side ="right",fill ="y")

            cstatus =tk .Label (cbody ,text ="",font =(FONT_FAMILY ,9 ),
            fg =MUTED ,bg =BG ,anchor ="w",wraplength =430 ,justify ="left")

            def refresh_list ():
                defs_list .delete (0 ,"end")
                entries =list_custom_defs ()
                if not entries :
                    defs_list .insert ("end","(no custom definition files loaded)")
                for e in entries :
                    kb =max (1 ,e ["size"]//1024 )
                    defs_list .insert ("end",
                    f"{e ['name']}   —   {e ['signatures']} signature(s), {kb } KB")
                cstatus .config (
                text =f"{len (entries )} custom definition file(s) loaded — "
                "these are merged into every scan automatically."
                if entries else "No custom definition files yet.")

            def do_add ():
                paths =filedialog .askopenfilenames (
                title ="Choose signature file(s) to add",
                filetypes =[("ClamAV / YARA signatures",
                "*.hdb *.hsb *.ndb *.ldb *.yar *.yara"),("All files","*.*")])
                if not paths :
                    return 
                added ,failed =0 ,[]
                for p in paths :
                    ok ,msg =add_custom_def_file (p )
                    if ok :
                        added +=1 
                    else :
                        failed .append (f"{os .path .basename (p )}: {msg }")
                refresh_list ()
                if failed :
                    ...
                    Toast (self ,"Definitions Added ✅",
                    f"{added } custom definition file(s) added.",ok =True )

            def do_remove ():
                sel =defs_list .curselection ()
                entries =list_custom_defs ()
                ...
                if not messagebox .askyesno ("Remove Definition",
                f"Remove \"{name }\" from custom definitions?",parent =cwin ):
                    return 
                remove_custom_def_file (name )
                refresh_list ()

            btn_row =tk .Frame (cbody ,bg =BG )
            FlatBtn (btn_row ,text ="➕ Add File(s)",base_bg =ACCENT2 ,fg ="#ffffff",
            command =do_add ,radius =12 ,height =32 ,width =120 ,
            font =(FONT_FAMILY ,9 ,"bold")).pack (side ="left",padx =(0 ,8 ))
            FlatBtn (btn_row ,text ="🗑 Remove Selected",base_bg =BORDER ,fg =TEXT ,
            command =do_remove ,radius =12 ,height =32 ,width =150 ,
            font =(FONT_FAMILY ,9 ,"bold")).pack (side ="left")
        ...
        custom_defs_btn =FlatBtn (action_row ,text ="📑 Custom Definitions",
        base_bg =BORDER ,fg =TEXT ,command =do_manage_custom_defs ,
        radius =12 ,height =32 ,width =160 ,font =(FONT_FAMILY ,9 ,"bold"))
        custom_defs_btn .pack (side ="left",padx =(8 ,0 ))

        def do_install ():
            if state .get ("installing")or state ["scanning"]:
                return 
            msg =("This downloads ClamAV straight from its official GitHub "
            "releases and installs it automatically")
            if os .name !="nt":
                msg +=(" — on this OS that means using your system's package "
                "manager (you may see a password prompt), or compiling it "
                "from source if no package manager is available")
            msg +=(". It can take a few minutes. Continue?")
            if not messagebox .askyesno ("Install ClamAV",msg ,parent =win ):
                return 

            state ["installing"]=True 
            install_holder ["btn"].set_disabled (True )
            install_holder ["btn"].config (text ="Installing…")
            scan_btn_holder ["btn"].set_disabled (True )
            status_lbl .config (text ="Installing ClamAV…",fg =MUTED )
            summary_lbl .config (text ="")
            log_text .configure (state ="normal")
            log_text .delete ("1.0","end")
            log_text .configure (state ="disabled")
            ...
            def worker ():
                ok ,msg2 =install_clamav (line_callback =on_line )
                def done ():
                    state ["installing"]=False 
                    if not win .winfo_exists ():
                        return 
                    install_holder ["btn"].config (text ="⚙  Install ClamAV")
                    install_holder ["btn"].set_disabled (False )
                    scan_btn_holder ["btn"].set_disabled (False )
                    state ["tool"]=find_clamscan ()
                    refresh_availability ()
                    if ok :
                        status_lbl .config (text ="ClamAV installed.",fg ="#3ddc97")
                        summary_lbl .config (text =f"✅  {msg2 }",fg ="#3ddc97")
                        Toast (self ,"ClamAV Installed ✅",msg2 ,ok =True )
                    else :
                        status_lbl .config (text ="Install failed — see the log.",fg =DANGER )
                        summary_lbl .config (text =f"🚫  {msg2 }",fg =DANGER )
                        Toast (self ,"Install Failed",msg2 ,ok =False )
                self .after (0 ,done )
            threading .Thread (target =worker ,daemon =True ).start ()

        install_holder ["fn"]=do_install 

        def refresh_availability ():
            if state ["tool"]:
                notice .pack_forget ()
                if not update_btn_shown ["v"]:
                    update_btn .pack (side ="left",padx =(8 ,0 ))
                    update_btn_shown ["v"]=True 
            else :
                update_btn .pack_forget ()
                update_btn_shown ["v"]=False 
                notice .pack (fill ="x",pady =(0 ,14 ),before =picker )

        update_btn_shown ={"v":False }
        update_btn =FlatBtn (action_row ,text ="⬆ Update Definitions",
        base_bg =BORDER ,fg =TEXT ,command =do_update_defs ,
        radius =12 ,height =32 ,width =150 ,font =(FONT_FAMILY ,9 ,"bold"))

        picker .pack (fill ="x",pady =(0 ,10 ))
        scan_btn_holder ["btn"].pack (side ="right")
        action_row .pack (fill ="x",pady =(0 ,10 ))
        log_frame .pack (fill ="both",expand =True ,pady =(0 ,10 ))
        log_text .pack (side ="left",fill ="both",expand =True )
        log_scroll .pack (side ="right",fill ="y")
        summary_lbl .pack (fill ="x",pady =(0 ,4 ),anchor ="w")

        def do_open_suggested_defs ():
            swin =tk .Toplevel (win )
            swin .title ("Suggested Definitions")
            swin .geometry ("500x560")
            swin .configure (bg =BG )
            swin .transient (win )
            apply_titlebar_theme (swin ,self .theme_data .get ("titlebar_color"))

            sbody =tk .Frame (swin ,bg =BG ,padx =18 ,pady =16 )
            sbody .pack (fill ="both",expand =True )

            tk .Label (sbody ,text ="✨  Suggested Definitions",
            font =(FONT_FAMILY ,13 ,"bold"),fg =TEXT ,bg =BG ,anchor ="w"
            ).pack (anchor ="w")
            tk .Label (sbody ,
            text ="Free, community-maintained ClamAV signature feeds "
            "covering advanced trojans, phishing/social-engineering "
            "kits, and coin-miner heuristics. One click to add or "
            "remove — or add your own list below.",
            font =(FONT_FAMILY ,9 ),fg =MUTED ,bg =BG ,anchor ="w",
            wraplength =450 ,justify ="left").pack (anchor ="w",pady =(4 ,10 ))

            scroll_wrap =tk .Frame (sbody ,bg =BG )
            scroll_wrap .pack (fill ="both",expand =True )
            s_canvas =tk .Canvas (scroll_wrap ,bg =BG ,highlightthickness =0 ,bd =0 )
            s_vsb =tk .Scrollbar (scroll_wrap ,orient ="vertical",command =s_canvas .yview )
            s_canvas .configure (yscrollcommand =s_vsb .set )
            s_vsb .pack (side ="right",fill ="y")
            s_canvas .pack (side ="left",fill ="both",expand =True )
            rows_frame =tk .Frame (s_canvas ,bg =BG )
            s_win_id =s_canvas .create_window ((0 ,0 ),window =rows_frame ,anchor ="nw")
            rows_frame .bind ("<Configure>",lambda e :
            s_canvas .configure (scrollregion =s_canvas .bbox ("all")))
            s_canvas .bind ("<Configure>",lambda e :
            s_canvas .itemconfig (s_win_id ,width =e .width ))

            def _s_wheel (event ):
                if getattr (event ,"num",None )==4 :
                    s_canvas .yview_scroll (-1 ,"units")
                elif getattr (event ,"num",None )==5 :
                    s_canvas .yview_scroll (1 ,"units")
                else :
                    s_canvas .yview_scroll (int (-event .delta /120 ),"units")
                return "break"

            def _s_bind_wheel (widget ):
                widget .bind ("<MouseWheel>",_s_wheel ,add ="+")
                widget .bind ("<Button-4>",_s_wheel ,add ="+")
                widget .bind ("<Button-5>",_s_wheel ,add ="+")
                for child in widget .winfo_children ():
                    _s_bind_wheel (child )

            for seq in ("<MouseWheel>","<Button-4>","<Button-5>"):
                s_canvas .bind (seq ,_s_wheel )


            def do_toggle_pua (row_btn ,row_desc ):
                new_state =not pua_detection_enabled ()
                set_pua_detection (new_state )
                row_btn .config (text ="🗑 Remove"if new_state else "➕ Add")
                row_desc .config (fg ="#3ddc97"if new_state else MUTED )
                Toast (self ,"PUA Detection Enabled ✅"if new_state else "PUA Detection Disabled",
                "ClamAV will now flag coin-miner / PUA-classified tools during scans."
                if new_state else "Coin-miner / PUA heuristics turned back off.",
                ok =True )

            def do_toggle_suggested (entry ,row_btn ,row_desc ):
                if suggested_def_installed (entry ):
                    remove_custom_def_file (entry ["filename"])
                    row_btn .config (text ="➕ Add",state ="normal")
                    row_desc .config (fg =MUTED )
                    Toast (self ,"Definition Removed",f"\"{entry ['name']}\" removed.",ok =True )
                    return 
                row_btn .config (text ="Downloading…",state ="disabled")

                def worker ():
                    ok ,msg =download_suggested_def (entry )
                    def done ():
                        if not swin .winfo_exists ():
                            return 
                        row_btn .config (state ="normal")
                        if ok :
                            row_btn .config (text ="🗑 Remove")
                            row_desc .config (fg ="#3ddc97")
                            Toast (self ,"Definition Added ✅",msg ,ok =True )
                        else :
                            row_btn .config (text ="➕ Add")
                            messagebox .showwarning ("Download Failed",msg ,parent =swin )
                    self .after (0 ,done )
                threading .Thread (target =worker ,daemon =True ).start ()

            def build_row (parent ,title ,category ,desc ,is_installed ,on_click ,
            removable_list =False ,on_delete_list =None ):
                row =tk .Frame (parent ,bg =CARD ,highlightthickness =1 ,
                highlightbackground =BORDER )
                inner =tk .Frame (row ,bg =CARD ,padx =12 ,pady =8 )
                inner .pack (fill ="x")
                top_row =tk .Frame (inner ,bg =CARD )
                top_row .pack (fill ="x")
                tk .Label (top_row ,text =title ,font =(FONT_FAMILY ,9 ,"bold"),
                fg =TEXT ,bg =CARD ,anchor ="w").pack (side ="left")
                tk .Label (top_row ,text =category ,font =(FONT_FAMILY ,8 ),
                fg =ACCENT2 ,bg =CARD ,anchor ="e").pack (side ="right")
                desc_lbl =tk .Label (inner ,text =desc ,font =(FONT_FAMILY ,8 ),
                fg =("#3ddc97"if is_installed else MUTED ),bg =CARD ,anchor ="w",
                wraplength =330 ,justify ="left")
                desc_lbl .pack (anchor ="w",fill ="x",pady =(2 ,6 ))
                btn_row =tk .Frame (inner ,bg =CARD )
                btn_row .pack (anchor ="e")
                if removable_list :
                    FlatBtn (btn_row ,text ="✕ Delete List",base_bg =CARD ,fg =DANGER ,
                    ghost =True ,border_col =DANGER ,radius =10 ,height =26 ,width =100 ,
                    font =(FONT_FAMILY ,8 ,"bold"),command =on_delete_list 
                    ).pack (side ="left",padx =(0 ,6 ))
                btn =FlatBtn (btn_row ,text =("🗑 Remove"if is_installed else "➕ Add"),
                base_bg =BORDER ,fg =TEXT ,radius =10 ,height =26 ,width =100 ,
                font =(FONT_FAMILY ,8 ,"bold"),command =lambda :on_click (btn ,desc_lbl ))
                btn .pack (side ="left")
            ...
            def refresh_rows ():
                for w in rows_frame .winfo_children ():
                    w .destroy ()
                for entry in all_suggested_defs ():
                    is_user =entry .get ("source")=="direct"
                    def make_delete (eid =entry .get ("id")):
                        def _del ():
                            if not messagebox .askyesno ("Delete List",
                            "Remove this suggested list entirely? (Any already-"
                            "downloaded definitions stay installed.)",parent =swin ):
                                return 
                            remove_user_suggested_list (eid )
                            refresh_rows ()
                        return _del 
                    build_row (rows_frame ,entry ["name"],entry ["category"],
                    entry ["desc"],suggested_def_installed (entry ),
                    (lambda e =entry :lambda b ,d :do_toggle_suggested (e ,b ,d ))(),
                    removable_list =is_user ,
                    on_delete_list =make_delete ()if is_user else None )
                build_row (rows_frame ,"Coin-Miner / PUA Heuristics","Cryptomining",
                "Turns on ClamAV's built-in \"Potentially Unwanted Application\" "
                "detection, which flags known coin-miner droppers and other "
                "PUA-classified tools during scans.",
                pua_detection_enabled (),
                lambda b ,d :do_toggle_pua (b ,d ))
                _s_bind_wheel (rows_frame )

            add_bar =tk .Frame (sbody ,bg =BG )
            add_bar .pack (fill ="x",pady =(10 ,0 ))

            def do_add_list ():
                fwin =tk .Toplevel (swin )
                fwin .title ("Add a Suggested List")
                fwin .geometry ("420x300")
                fwin .configure (bg =BG )
                fwin .transient (swin )
                apply_titlebar_theme (fwin ,self .theme_data .get ("titlebar_color"))
                fb =tk .Frame (fwin ,bg =BG ,padx =18 ,pady =16 )
                fb .pack (fill ="both",expand =True )
                tk .Label (fb ,text ="Add a Suggested List",
                font =(FONT_FAMILY ,12 ,"bold"),fg =TEXT ,bg =BG ,anchor ="w"
                ).pack (anchor ="w",pady =(0 ,2 ))
                tk .Label (fb ,text ="Must be a direct link to a real ClamAV/YARA "
                "signature file (.hdb .hsb .ndb .ldb .yar .yara).",
                font =(FONT_FAMILY ,8 ),fg =MUTED ,bg =BG ,anchor ="w",
                wraplength =380 ,justify ="left").pack (anchor ="w",pady =(0 ,10 ))

                def labeled_entry (label ):
                    tk .Label (fb ,text =label ,font =(FONT_FAMILY ,9 ),
                    fg =MUTED ,bg =BG ,anchor ="w").pack (anchor ="w")
                    var =tk .StringVar ()
                    e =tk .Entry (fb ,textvariable =var ,font =(FONT_FAMILY ,10 ),
                    bg =CARD ,fg =TEXT ,insertbackground =TEXT ,relief ="flat")
                    e .pack (fill ="x",ipady =5 ,pady =(2 ,10 ))
                    return var 

                name_var =labeled_entry ("List name")
                category_var =labeled_entry ("Category (e.g. Advanced Trojans & Malware)")
                url_var =labeled_entry ("Direct URL to the signature file")

                fstatus =tk .Label (fb ,text ="",font =(FONT_FAMILY ,8 ),
                fg =DANGER ,bg =BG ,anchor ="w",wraplength =380 ,justify ="left")
                fstatus .pack (anchor ="w",pady =(0 ,8 ))

                def do_save ():
                    ok ,msg =add_user_suggested_list (
                    name_var .get (),category_var .get (),url_var .get ())
                    if not ok :
                        fstatus .config (text =msg )
                        return 
                    fwin .destroy ()
                    refresh_rows ()
                    Toast (self ,"List Added ✅",msg ,ok =True )

                FlatBtn (fb ,text ="➕ Add List",base_bg =ACCENT2 ,fg ="#ffffff",
                command =do_save ,radius =12 ,height =32 ,width =120 ,
                font =(FONT_FAMILY ,9 ,"bold")).pack (anchor ="e")

            FlatBtn (add_bar ,text ="➕ Add a List",base_bg =BORDER ,fg =TEXT ,
            command =do_add_list ,radius =12 ,height =32 ,width =130 ,
            font =(FONT_FAMILY ,9 ,"bold")).pack (side ="left")

            refresh_rows ()

        suggested_defs_btn =FlatBtn (body ,text ="✨ Suggested Definitions",
        base_bg =BORDER ,fg =TEXT ,command =do_open_suggested_defs ,
        radius =12 ,height =34 ,width =220 ,font =(FONT_FAMILY ,9 ,"bold"))
        suggested_defs_btn .pack (anchor ="w",pady =(10 ,0 ))
    ...
    def _open_breach_check (self ):
        ...
        tk .Label (body ,text ="🔓  Breach Check",
        font =(FONT_FAMILY ,16 ,"bold"),fg =TEXT ,bg =BG ,anchor ="w"
        ).pack (anchor ="w")
        tk .Label (body ,
        text ="Password and site checks use Have I Been Pwned; email and "
        "username checks use LeakCheck — all four are free and need no "
        "setup. Password checks never send your real password anywhere, "
        "only a short hash fragment.",
        font =(FONT_FAMILY ,9 ),fg =MUTED ,bg =BG ,anchor ="w",
        wraplength =470 ,justify ="left").pack (anchor ="w",pady =(2 ,14 ))

        tab_row =tk .Frame (body ,bg =BG )
        tab_row .pack (fill ="x",pady =(0 ,12 ))
        panels_holder ={"frame":None }
        tab_btns ={}
        ...
        for name in ("Password","Email","Site / Domain","Username","Batch"):
            b =tk .Label (tab_row ,text =name ,font =(FONT_FAMILY ,9 ,"bold"),
            fg =MUTED ,bg =BG ,padx =12 ,pady =6 ,cursor ="hand2")
            b .pack (side ="left",padx =(0 ,4 ))
            b .bind ("<Button-1>",lambda e ,n =name :show_tab (n ))
            tab_btns [name ]=b 
        ...
        result_font =(FONT_FAMILY ,9 )

        pw_panel =tk .Frame (content_area ,bg =BG )
        panels ["Password"]=pw_panel
        tk .Label (pw_panel ,text ="Password",font =(FONT_FAMILY ,9 ),
        fg =MUTED ,bg =BG ,anchor ="w").pack (anchor ="w")
        pw_var =tk .StringVar ()
        pw_row =tk .Frame (pw_panel ,bg =BG )
        pw_row .pack (fill ="x",pady =(2 ,10 ))
        pw_entry =tk .Entry (pw_row ,textvariable =pw_var ,show ="•",
        font =(FONT_FAMILY ,10 ),bg =CARD ,fg =TEXT ,insertbackground =TEXT ,relief ="flat")
        pw_entry .pack (side ="left",fill ="x",expand =True ,ipady =6 ,padx =(0 ,8 ))
        show_var =tk .BooleanVar (value =False )
        def toggle_show ():
            pw_entry .config (show =""if show_var .get ()else "•")
        tk .Checkbutton (pw_row ,text ="Show",variable =show_var ,command =toggle_show ,
        bg =BG ,fg =MUTED ,selectcolor =CARD ,activebackground =BG ,
        font =(FONT_FAMILY ,8 )).pack (side ="left")

        pw_result =tk .Label (pw_panel ,text ="",font =result_font ,bg =BG ,
        anchor ="w",wraplength =460 ,justify ="left")
        pw_result .pack (anchor ="w",fill ="x")
        pw_btn_holder ={"btn":None }

        def do_check_password ():
            pw_btn_holder ["btn"].set_disabled (True )
            pw_btn_holder ["btn"].config (text ="Checking…")
            pw_result .config (text ="",fg =MUTED )
            pwd =pw_var .get ()
            def worker ():
                ok ,count ,err =check_password_pwned (pwd )
                def done ():
                    if not win .winfo_exists ():
                        return 
                    pw_btn_holder ["btn"].set_disabled (False )
                    pw_btn_holder ["btn"].config (text ="🔍  Check Password")
                    if not ok :
                        pw_result .config (text =f"⚠️  {err }",fg =WARN )
                    elif count :
                        pw_result .config (
                        text =f"🚫  Found in {count :,} known breaches — do not use this password.",
                        fg =DANGER )
                    else :
                        pw_result .config (
                        text ="✅  Not found in any known breach.",fg ="#3ddc97")
                self .after (0 ,done )
            threading .Thread (target =worker ,daemon =True ).start ()

        pw_btn_holder ["btn"]=FlatBtn (pw_panel ,text ="🔍  Check Password",
        base_bg =ACCENT2 ,fg ="#ffffff",command =do_check_password ,
        radius =12 ,height =32 ,width =160 ,font =(FONT_FAMILY ,9 ,"bold"))
        pw_btn_holder ["btn"].pack (anchor ="w",pady =(8 ,0 ))

        em_panel =tk .Frame (content_area ,bg =BG )
        panels ["Email"]=em_panel
        tk .Label (em_panel ,
        text ="Free, instant lookup — no account or API key needed. "
        "Powered by LeakCheck's public API, which only tells you which "
        "breaches an email turned up in, never any leaked data itself.",
        font =(FONT_FAMILY ,8 ),fg =MUTED ,bg =BG ,anchor ="w",
        wraplength =460 ,justify ="left").pack (anchor ="w",pady =(0 ,8 ))

        tk .Label (em_panel ,text ="Email Address",font =(FONT_FAMILY ,9 ),
        fg =MUTED ,bg =BG ,anchor ="w").pack (anchor ="w")
        email_var =tk .StringVar ()
        email_entry =tk .Entry (em_panel ,textvariable =email_var ,font =(FONT_FAMILY ,10 ),
        bg =CARD ,fg =TEXT ,insertbackground =TEXT ,relief ="flat")
        email_entry .pack (fill ="x",ipady =6 ,pady =(2 ,10 ))

        def format_breach_report (found ,sources ,fields ):
            lines =[f"🚫  Found in {found } breach(es):"]
            shown =sources [:12 ]
            for s in shown :
                name =s .get ("name","?")
                date =s .get ("date","")
                lines .append ("   • "+name +(f"  ({date })"if date else ""))
            if len (sources )>len (shown ):
                lines .append (f"   …and {len (sources )-len (shown )} more.")
            return "\n".join (lines )

        em_result =tk .Label (em_panel ,text ="",font =result_font ,bg =BG ,
        anchor ="w",wraplength =460 ,justify ="left")
        em_result .pack (anchor ="w",fill ="x")
        em_btn_holder ={"btn":None }

        def do_check_email ():
            em_btn_holder ["btn"].set_disabled (True )
            em_btn_holder ["btn"].config (text ="Checking…")
            em_result .config (text ="",fg =MUTED )
            email =email_var .get ().strip ()
            def worker ():
                ok ,found ,sources ,fields ,err =check_email_breaches (email )
                def done ():
                    if not win .winfo_exists ():
                        return 
                    em_btn_holder ["btn"].set_disabled (False )
                    em_btn_holder ["btn"].config (text ="🔍  Check Email")
                    if not ok :
                        em_result .config (text =f"⚠️  {err }",fg =WARN )
                    elif found :
                        em_result .config (
                        text =format_breach_report (found ,sources ,fields ),
                        fg =DANGER )
                    else :
                        em_result .config (
                        text ="✅  No breaches found for this email.",fg ="#3ddc97")
                self .after (0 ,done )
            threading .Thread (target =worker ,daemon =True ).start ()

        em_btn_holder ["btn"]=FlatBtn (em_panel ,text ="🔍  Check Email",
        base_bg =ACCENT2 ,fg ="#ffffff",command =do_check_email ,
        radius =12 ,height =32 ,width =150 ,font =(FONT_FAMILY ,9 ,"bold"))
        em_btn_holder ["btn"].pack (anchor ="w",pady =(8 ,0 ))
        tk .Label (em_panel ,text ="Powered by LeakCheck",
        font =(FONT_FAMILY ,7 ),fg =MUTED ,bg =BG ,anchor ="w"
        ).pack (anchor ="w",pady =(10 ,0 ))

        site_panel =tk .Frame (content_area ,bg =BG )
        panels ["Site / Domain"]=site_panel
        tk .Label (site_panel ,
        text ="Looks up whether a given online service/website has ever "
        "disclosed a breach — this is public breach history for the "
        "service itself, not a check on any individual's account.",
        font =(FONT_FAMILY ,8 ),fg =MUTED ,bg =BG ,anchor ="w",
        wraplength =460 ,justify ="left").pack (anchor ="w",pady =(0 ,8 ))
        tk .Label (site_panel ,text ="Domain (e.g. adobe.com)",
        font =(FONT_FAMILY ,9 ),fg =MUTED ,bg =BG ,anchor ="w").pack (anchor ="w")
        domain_var =tk .StringVar ()
        tk .Entry (site_panel ,textvariable =domain_var ,font =(FONT_FAMILY ,10 ),
        bg =CARD ,fg =TEXT ,insertbackground =TEXT ,relief ="flat"
        ).pack (fill ="x",ipady =6 ,pady =(2 ,10 ))

        site_result =tk .Label (site_panel ,text ="",font =result_font ,bg =BG ,
        anchor ="w",wraplength =460 ,justify ="left")
        site_result .pack (anchor ="w",fill ="x")
        site_btn_holder ={"btn":None }

        def do_check_domain ():
            site_btn_holder ["btn"].set_disabled (True )
            site_btn_holder ["btn"].config (text ="Checking…")
            site_result .config (text ="",fg =MUTED )
            domain =domain_var .get ()
            def worker ():
                ok ,breaches ,err =check_domain_breaches (domain )
                def done ():
                    if not win .winfo_exists ():
                        return 
                    site_btn_holder ["btn"].set_disabled (False )
                    site_btn_holder ["btn"].config (text ="🔍  Check Site")
                    if not ok :
                        site_result .config (text =f"⚠️  {err }",fg =WARN )
                    elif breaches :
                        names =", ".join (b .get ("Name","?")for b in breaches )
                        site_result .config (
                        text =f"🚫  This service has disclosed {len (breaches )} "
                        f"breach(es): {names }",fg =DANGER )
                    else :
                        site_result .config (
                        text ="✅  No known breaches on record for this domain.",
                        fg ="#3ddc97")
                self .after (0 ,done )
            threading .Thread (target =worker ,daemon =True ).start ()

        site_btn_holder ["btn"]=FlatBtn (site_panel ,text ="🔍  Check Site",
        base_bg =ACCENT2 ,fg ="#ffffff",command =do_check_domain ,
        radius =12 ,height =32 ,width =140 ,font =(FONT_FAMILY ,9 ,"bold"))
        site_btn_holder ["btn"].pack (anchor ="w",pady =(8 ,0 ))

        user_panel =tk .Frame (content_area ,bg =BG )
        panels ["Username"]=user_panel
        tk .Label (user_panel ,
        text ="Free, instant lookup — no account or API key needed. "
        "Checks whether a username has turned up in any known breach "
        "(same LeakCheck public API as the Email tab, just auto-detecting "
        "a username instead of an address).",
        font =(FONT_FAMILY ,8 ),fg =MUTED ,bg =BG ,anchor ="w",
        wraplength =460 ,justify ="left").pack (anchor ="w",pady =(0 ,8 ))

        tk .Label (user_panel ,text ="Username",font =(FONT_FAMILY ,9 ),
        fg =MUTED ,bg =BG ,anchor ="w").pack (anchor ="w")
        user_var =tk .StringVar ()
        tk .Entry (user_panel ,textvariable =user_var ,font =(FONT_FAMILY ,10 ),
        bg =CARD ,fg =TEXT ,insertbackground =TEXT ,relief ="flat"
        ).pack (fill ="x",ipady =6 ,pady =(2 ,10 ))

        user_result =tk .Label (user_panel ,text ="",font =result_font ,bg =BG ,
        anchor ="w",wraplength =460 ,justify ="left")
        user_result .pack (anchor ="w",fill ="x")
        user_btn_holder ={"btn":None }

        def do_check_username ():
            user_btn_holder ["btn"].set_disabled (True )
            user_btn_holder ["btn"].config (text ="Checking…")
            user_result .config (text ="",fg =MUTED )
            username =user_var .get ().strip ()
            def worker ():
                ok ,found ,sources ,fields ,err =check_username_breaches (username )
                def done ():
                    if not win .winfo_exists ():
                        return 
                    user_btn_holder ["btn"].set_disabled (False )
                    user_btn_holder ["btn"].config (text ="🔍  Check Username")
                    if not ok :
                        user_result .config (text =f"⚠️  {err }",fg =WARN )
                    elif found :
                        user_result .config (
                        text =format_breach_report (found ,sources ,fields ),
                        fg =DANGER )
                        ...
                        user_result .config (
                        text ="✅  No breaches found for this username.",fg ="#3ddc97")
                self .after (0 ,done )
            threading .Thread (target =worker ,daemon =True ).start ()

        user_btn_holder ["btn"]=FlatBtn (user_panel ,text ="🔍  Check Username",
        base_bg =ACCENT2 ,fg ="#ffffff",command =do_check_username ,
        radius =12 ,height =32 ,width =160 ,font =(FONT_FAMILY ,9 ,"bold"))
        user_btn_holder ["btn"].pack (anchor ="w",pady =(8 ,0 ))
        tk .Label (user_panel ,text ="Powered by LeakCheck",
        font =(FONT_FAMILY ,7 ),fg =MUTED ,bg =BG ,anchor ="w"
        ).pack (anchor ="w",pady =(10 ,0 ))

        batch_panel =tk .Frame (content_area ,bg =BG )
        panels ["Batch"]=batch_panel

        tk .Label (batch_panel ,
        text ="Check a whole list at once — one entry per line. Paste them "
        "in below or load a .txt file (handy for checking everyone in the "
        "household in one go). Works for any of the four check types; "
        ...
        BATCH_TYPES ={
        "Email":{
        "hint":"Emails, one per line",
        "ci":True ,
        "validate":lambda v :"@"in v ,
        "invalid_msg":"not a valid email",
        "sleep":1.1 ,
        "credit":"Powered by LeakCheck",
        },
        "Username":{
        "hint":"Usernames, one per line",
        "ci":True ,
        ...
        "Password":{
        "hint":"Passwords, one per line — never shown in the log/report, "
        "and only a hash prefix is ever sent (see the Password tab).",
        "ci":False ,
        "validate":lambda v :True ,
        "invalid_msg":"",
        "sleep":0.3 ,
        "credit":"Powered by Have I Been Pwned",
        },
        "Site / Domain":{
        "hint":"Domains, one per line (e.g. adobe.com)",
        "ci":True ,
        "validate":lambda v :"."in v ,
        "invalid_msg":"not a valid domain",
        "sleep":1.3 ,
        "credit":"Powered by Have I Been Pwned",
        },
        }

        batch_type_var =tk .StringVar (value ="Email")
        type_row =tk .Frame (batch_panel ,bg =BG )
        type_row .pack (fill ="x",pady =(0 ,8 ))
        tk .Label (type_row ,text ="Check type:",font =(FONT_FAMILY ,9 ),
        fg =MUTED ,bg =BG ).pack (side ="left",padx =(0 ,10 ))
        for opt in ("Email","Username","Password","Site / Domain"):
            tk .Radiobutton (type_row ,text =opt ,variable =batch_type_var ,value =opt ,
            command =lambda :batch_on_type_change (),
            bg =BG ,fg =MUTED ,selectcolor =CARD ,activebackground =BG ,
            activeforeground =TEXT ,font =(FONT_FAMILY ,9 ),
            highlightthickness =0 ).pack (side ="left",padx =(0 ,10 ))

        batch_input_frame =tk .Frame (batch_panel ,bg =CARD ,highlightthickness =1 ,
        highlightbackground =BORDER )
        batch_input_frame .pack (fill ="x",pady =(0 ,4 ))
        batch_input =tk .Text (batch_input_frame ,bg =CARD ,fg =TEXT ,
        insertbackground =TEXT ,font =(FONT_FAMILY ,9 ),relief ="flat",
        height =5 ,wrap ="none")
        batch_input .pack (fill ="both",expand =True ,padx =6 ,pady =6 )

        batch_hint_lbl =tk .Label (batch_panel ,text ="",
        font =(FONT_FAMILY ,8 ),fg =MUTED ,bg =BG ,anchor ="w",
        wraplength =520 ,justify ="left")
        batch_hint_lbl .pack (anchor ="w",pady =(2 ,4 ))

        batch_file_lbl =tk .Label (batch_panel ,text ="No file loaded.",
        font =(FONT_FAMILY ,8 ),fg =MUTED ,bg =BG ,anchor ="w")
        batch_file_lbl .pack (anchor ="w",pady =(0 ,8 ))

        def batch_on_type_change ():
            cfg =BATCH_TYPES [batch_type_var .get ()]
            batch_hint_lbl .config (text =cfg ["hint"])
            batch_file_lbl .config (text ="No file loaded.")
            batch_credit_lbl .config (text =cfg ["credit"])

        def batch_load_file ():
            p =filedialog .askopenfilename (
            title =f"Choose a .txt file of {batch_type_var .get ().lower ()} entries (one per line)",
            filetypes =[("Text files","*.txt"),("All files","*.*")])
            if not p :
                return 
            try :
                ...
                batch_input .insert ("1.0",content )
                batch_file_lbl .config (text =f"Loaded: {os .path .basename (p )}")
            except Exception as e :
                Toast (self ,"Couldn't Load File",str (e ),ok =False )

        batch_top_row =tk .Frame (batch_panel ,bg =BG )
        batch_top_row .pack (fill ="x",pady =(0 ,10 ))
        FlatBtn (batch_top_row ,text ="📄  Load .txt File",base_bg =BORDER ,fg =TEXT ,
        command =batch_load_file ,radius =10 ,height =32 ,width =150 ,
        font =(FONT_FAMILY ,9 ,"bold")).pack (side ="left",padx =(0 ,8 ))

        batch_status_lbl =tk .Label (batch_top_row ,text ="",font =(FONT_FAMILY ,9 ),
        fg =MUTED ,bg =BG ,anchor ="w")
        batch_status_lbl .pack (side ="left",fill ="x",expand =True )

        batch_log_frame =tk .Frame (batch_panel ,bg =CARD ,highlightthickness =1 ,
        highlightbackground =BORDER )
        batch_log_frame .pack (fill ="both",expand =True ,pady =(0 ,10 ))
        batch_log =tk .Text (batch_log_frame ,bg =CARD ,fg =MUTED ,
        font =("Consolas"if os .name =="nt"else "Courier",9 ),
        relief ="flat",wrap ="none",height =10 ,state ="disabled")
        batch_log_scroll =tk .Scrollbar (batch_log_frame ,command =batch_log .yview )
        batch_log .configure (yscrollcommand =batch_log_scroll .set )
        batch_log .tag_config ("bad",foreground =DANGER )
        batch_log .tag_config ("ok",foreground ="#3ddc97")
        batch_log .tag_config ("warn",foreground =WARN )
        batch_log .pack (side ="left",fill ="both",expand =True ,padx =(6 ,0 ),pady =6 )
        batch_log_scroll .pack (side ="right",fill ="y",pady =6 ,padx =(0 ,4 ))

        def batch_append (line ,tag =None ):
            batch_log .configure (state ="normal")
            batch_log .insert ("end",line +"\n",(tag ,)if tag else ())
            batch_log .see ("end")
            batch_log .configure (state ="disabled")

        batch_state ={"cancel":False }
        batch_btn_holder ={"btn":None }
        batch_stop_holder ={"btn":None }
        batch_credit_lbl =tk .Label (batch_panel ,text ="",
        font =(FONT_FAMILY ,7 ),fg =MUTED ,bg =BG ,anchor ="w")

        def batch_stop ():
            batch_state ["cancel"]=True 
            batch_status_lbl .config (text ="Stopping…")

        def batch_save_report ():
            content =batch_log .get ("1.0","end").strip ()
            if not content :
                Toast (self ,"Nothing To Save","Run a batch check first.",ok =False )
                return 
            p =filedialog .asksaveasfilename (defaultextension =".txt",
            filetypes =[("Text files","*.txt")],
            initialfile ="revolt_breach_batch_report.txt")
            if not p :
                return 
            try :
                with open (p ,"w",encoding ="utf-8")as f :
                    f .write (content +"\n")
                Toast (self ,"Report Saved",f"Saved to {os .path .basename (p )}.",ok =True )
            except Exception as e :
                Toast (self ,"Couldn't Save Report",str (e ),ok =False )

        def batch_check_one (check_type ,value ):
            """Runs the right backend check and returns a normalized
            (ok, hit_count, detail, err) — detail is a ready-to-print
            description, never the raw leaked data or the input secret."""
            if check_type =="Email":
                ok ,found ,sources ,fields ,err =check_email_breaches (value )
                if ok and found :
                    names =", ".join (s .get ("name","?")for s in sources [:6 ])
                    more ="" if len (sources )<=6 else f" +{len (sources )-6 } more"
                    return ok ,found ,f"{found } breach(es): {names }{more }",err 
                return ok ,found ,"",err 
            if check_type =="Username":
                ok ,found ,sources ,fields ,err =check_username_breaches (value )
                if ok and found :
                    names =", ".join (s .get ("name","?")for s in sources [:6 ])
                    more ="" if len (sources )<=6 else f" +{len (sources )-6 } more"
                    return ok ,found ,f"{found } breach(es): {names }{more }",err 
                return ok ,found ,"",err 
            if check_type =="Site / Domain":
                ok ,breaches ,err =check_domain_breaches (value )
                found =len (breaches )
                if ok and found :
                    names =", ".join (b .get ("Name","?")for b in breaches [:6 ])
                    more ="" if found <=6 else f" +{found -6 } more"
                    return ok ,found ,f"{found } breach(es) disclosed: {names }{more }",err 
                return ok ,found ,"",err 
            ok ,count ,err =check_password_pwned (value )
            if ok and count :
                return ok ,count ,f"seen {count :,} times in known breaches",err 
            return ok ,count ,"",err 

        def batch_run ():
            check_type =batch_type_var .get ()
            cfg =BATCH_TYPES [check_type ]
            raw =batch_input .get ("1.0","end")
            items =[]
            seen =set ()
            for line in raw .splitlines ():
                v =line .strip ("\r\n")
                v =v .strip ()if cfg ["ci"]else v .rstrip ()
                if not v :
                    continue 
                key =v .lower ()if cfg ["ci"]else v 
                if key in seen :
                    continue 
                seen .add (key )
                items .append (v )
            if not items :
                Toast (self ,"Nothing To Check",
                "Paste or load some entries first, one per line.",ok =False )
                return 

            batch_log .configure (state ="normal")
            batch_log .delete ("1.0","end")
            batch_log .configure (state ="disabled")
            batch_state ["cancel"]=False 
            batch_btn_holder ["btn"].set_disabled (True )
            batch_btn_holder ["btn"].config (text ="Running…")
            batch_stop_holder ["btn"].pack (side ="left",padx =(8 ,0 ))
            total =len (items )
            tally ={"found":0 ,"clean":0 ,"errors":0 }
            is_password =check_type =="Password"

            def worker ():
                for i ,value in enumerate (items ,start =1 ):
                    if batch_state ["cancel"]:
                        break 
                    label =f"Entry #{i }"if is_password else value 

                    if not cfg ["validate"](value ):
                        def skip (label =label ,msg =cfg ["invalid_msg"]):
                            batch_append (f"⚠️  {label } — skipped ({msg })","warn")
                        self .after (0 ,skip )
                        continue 

                    def progress (i =i ,label =label ):
                        batch_status_lbl .config (text =f"Checking {i }/{total }: {label }")
                    self .after (0 ,progress )

                    ok ,hits ,detail ,err =batch_check_one (check_type ,value )

                    def report (label =label ,ok =ok ,hits =hits ,detail =detail ,err =err ):
                        if not ok :
                            batch_append (f"⚠️  {label } — {err }","warn")
                            tally ["errors"]+=1 
                        elif hits :
                            batch_append (f"🚫  {label } — {detail }","bad")
                            tally ["found"]+=1 
                        else :
                            batch_append (f"✅  {label } — clean","ok")
                            tally ["clean"]+=1 
                    self .after (0 ,report )

                    if i <total and not batch_state ["cancel"]:
                        time .sleep (cfg ["sleep"])

                def finish ():
                    if not win .winfo_exists ():
                        return 
                    batch_btn_holder ["btn"].set_disabled (False )
                    batch_btn_holder ["btn"].config (text ="▶️  Run Batch Check")
                    batch_stop_holder ["btn"].pack_forget ()
                    if batch_state ["cancel"]:
                        batch_status_lbl .config (
                        text =f"Stopped — {tally ['found']} found, {tally ['clean']} clean.")
                    else :
                        batch_status_lbl .config (
                        text =f"Done — {tally ['found']} found, {tally ['clean']} clean, "
                        f"{tally ['errors']} error(s).")
                self .after (0 ,finish )

            threading .Thread (target =worker ,daemon =True ).start ()

        batch_action_row =tk .Frame (batch_panel ,bg =BG )
        batch_action_row .pack (fill ="x")
        batch_btn_holder ["btn"]=FlatBtn (batch_action_row ,text ="▶️  Run Batch Check",
        base_bg =ACCENT2 ,fg ="#ffffff",command =batch_run ,
        radius =12 ,height =32 ,width =170 ,font =(FONT_FAMILY ,9 ,"bold"))
        batch_btn_holder ["btn"].pack (side ="left")
        batch_stop_holder ["btn"]=FlatBtn (batch_action_row ,text ="⏹  Stop",
        base_bg =BORDER ,fg =TEXT ,command =batch_stop ,
        radius =12 ,height =32 ,width =90 ,font =(FONT_FAMILY ,9 ,"bold"))
        FlatBtn (batch_action_row ,text ="💾  Save Report",base_bg =CARD ,
        hover_bg =lighten (CARD ,.08 ),fg =MUTED ,ghost =True ,
        border_col =lighten (BORDER ,.2 ),command =batch_save_report ,
        radius =12 ,height =32 ,width =130 ,font =(FONT_FAMILY ,9 ,"bold")
        ).pack (side ="left",padx =(8 ,0 ))

        batch_credit_lbl .pack (anchor ="w",pady =(8 ,0 ))
        batch_on_type_change ()
    ...
    def _vpn_guard_tick (self ):
        ...
        def worker ():
            ...
            if ifaces :
                ...
                def done ():
                    if killed :
                        if getattr (self ,"_vpn_rotate_active",False ):
                            self ._stop_vpn_rotation (silent =True )
    ...
    def _resume_vpn_rotation_if_needed (self ):
        try :
            rcfg =load_vpn_rotate_config ()
            if not rcfg .get ("enabled"):
                return 
            vcfg =load_vpn_config ()
            if vcfg .get ("kind")!="openvpn"or not vpn_is_active ():
                save_vpn_rotate_config ({**rcfg ,"enabled":False })
                return 
            interval =max (5 ,min (240 ,int (rcfg .get ("interval_min")or 30 )))
            next_at =rcfg .get ("next_at")
            if not next_at or next_at <time .time ():
                next_at =time .time ()+interval *60 
            self ._start_vpn_rotation (interval ,next_at =next_at ,silent =True )
        except Exception :
            pass 

    def _start_vpn_rotation (self ,interval_min :int ,next_at :float |None =None ,
    silent :bool =False ):
        interval_min =max (5 ,min (240 ,int (interval_min )))
        self ._vpn_rotate_active =True 
        self ._vpn_rotate_interval_min =interval_min 
        self ._vpn_rotate_next_at =next_at if next_at else (time .time ()+interval_min *60 )
        save_vpn_rotate_config ({"enabled":True ,"interval_min":interval_min ,
        "next_at":self ._vpn_rotate_next_at })
        self ._vpn_rotate_tick ()
        if not silent :
            self ._fire ("VPN Chain Started 🔗",
            f"Auto-rotating to a new free relay every {interval_min } "
            f"minutes.",ok =True )

    def _stop_vpn_rotation (self ,silent :bool =False ):
        self ._vpn_rotate_active =False 
        if self ._vpn_rotate_after_id is not None :
            try :self .after_cancel (self ._vpn_rotate_after_id )
            except Exception :pass 
            self ._vpn_rotate_after_id =None 
        rcfg =load_vpn_rotate_config ()
        rcfg ["enabled"]=False 
        save_vpn_rotate_config (rcfg )
        if not silent :
            self ._fire ("VPN Chain Stopped",
            "Auto-rotate turned off — staying on the current relay.",
            ok =False )

    def _vpn_rotate_tick (self ):
        if not self ._vpn_rotate_active :
            return 
        remaining =self ._vpn_rotate_next_at -time .time ()
        if remaining <=0 :
            self ._perform_vpn_rotation ()
            return 
        wait_ms =max (50 ,min (1000 ,int (remaining *1000 )))
        self ._vpn_rotate_after_id =self .after (wait_ms ,self ._vpn_rotate_tick )

    def _perform_vpn_rotation (self ):
        if not self ._vpn_rotate_active :
            return 
        if not vpn_is_active ():
            self ._stop_vpn_rotation (silent =True )
            return 
        vcfg =load_vpn_config ()
        if vcfg .get ("kind")!="openvpn":
            self ._stop_vpn_rotation (silent =True )
            return 
        current_path =vcfg .get ("config_path")
        exclude_ip =self ._vpn_rotate_state .get ("current_ip")

        def worker ():
            relays =fetch_vpngate_relays ()
            chosen =_pick_random_vpngate_relay (relays ,exclude_ip =exclude_ip )
            if not chosen :
                result =("fail","Couldn't reach VPN Gate for a fresh "
                "relay list — will try again at the next hop.")
            else :
                new_path =save_vpngate_config (chosen )
                if not new_path :
                    result =("fail","The next relay's config was invalid "
                    "— will try again at the next hop.")
                else :
                    disconnect_vpn (current_path ,"openvpn")
                    ok ,msg =connect_vpn (new_path ,"openvpn")
                    if ok :
                        save_vpn_config ({"config_path":new_path ,"kind":"openvpn"})
                        result =("ok",chosen )
                    else :
                        connect_vpn (current_path ,"openvpn")
                        result =("fail",f"Couldn't connect to the new "
                        f"relay ({msg }) — stayed on the previous one.")

            def done ():
                if result [0 ]=="ok":
                    relay =result [1 ]
                    self ._vpn_rotate_state ["current_ip"]=relay .get ("ip")
                    place =relay .get ("country_long")or "Unknown"
                    self ._fire ("VPN Chain Rotated 🔗",
                    f"Hopped to a new relay in {place } "
                    f"({relay .get ('ip','?')}).",ok =True )
                else :
                    self ._fire ("VPN Chain Error",result [1 ],ok =False )
                self ._refresh_vpn_btn_label ()
                self ._vpn_location_cache ["ip"]=None 
                if self ._vpn_rotate_active :
                    self ._vpn_rotate_next_at =time .time ()+self ._vpn_rotate_interval_min *60 
                    save_vpn_rotate_config ({"enabled":True ,
                    "interval_min":self ._vpn_rotate_interval_min ,
                    "next_at":self ._vpn_rotate_next_at })
                    self ._vpn_rotate_tick ()
            self .after (0 ,done )
        threading .Thread (target =worker ,daemon =True ).start ()
    ...
    def _refresh_vpn_btn_label (self ):
        ...
        if active :
            chain_tag ="🔗 "if getattr (self ,"_vpn_rotate_active",False )else ""
            self .vpn_btn .config (text =f"🔓  Disconnect VPN {chain_tag}".rstrip (),
            fg ="#ff6b6b")
        else :
            self .vpn_btn .config (text ="🔒  Connect VPN",fg =TEXT )
        self ._refresh_vpn_location (active )

    def _refresh_vpn_location (self ,active :bool |None =None ):
        if not hasattr (self ,"vpn_location_lbl"):
            return 
        if active is None :
            active =vpn_is_active ()
        if not active :
            self ._vpn_location_cache ["ip"]=None 
            self .vpn_location_lbl .config (text ="")
            return 
        if self ._vpn_location_cache .get ("ip")is not None :
            return 
        self .vpn_location_lbl .config (text ="📍  Locating…",fg =MUTED )
        def worker ():
            loc =fetch_vpn_location ()
            def done ():
                if not hasattr (self ,"vpn_location_lbl")or not self .vpn_location_lbl .winfo_exists ():
                    return 
                if not vpn_is_active ():
                    self ._vpn_location_cache ["ip"]=None 
                    self .vpn_location_lbl .config (text ="")
                ...
                if loc :
                    ...
                    self ._vpn_location_cache ["ip"]=loc .get ("ip")or "unknown"
                    self .vpn_location_lbl .config (
                    text =f"📍  {place or 'Unknown location'}",fg ="#3ddc97")
                else :
                    self ._vpn_location_cache ["ip"]=None 
                    self .vpn_location_lbl .config (text ="📍  Location unavailable",fg =MUTED )
            self .after (0 ,done )
        threading .Thread (target =worker ,daemon =True ).start ()
    ...
    def _open_vpn_dialog (self ):
        ...
        def refresh_kind_buttons ():
            ...
            if not wg_on and not openvpn_install_state ["available"]:
                openvpn_notice .pack (fill ="x",pady =(10 ,0 ),before =config_hdr_lbl )
            else :
                openvpn_notice .pack_forget ()
            refresh_rotate_ui ()
        ...
        openvpn_install_state ={"installing":False ,
        "available":_detect_vpn_tool ("openvpn")is not None }

        openvpn_notice =tk .Frame (body ,bg =CARD ,highlightthickness =1 ,
        highlightbackground =WARN )
        openvpn_notice_inner =tk .Frame (openvpn_notice ,bg =CARD ,padx =14 ,pady =10 )
        openvpn_notice_inner .pack (fill ="x")
        tk .Label (openvpn_notice_inner ,
        text ="⚠  OpenVPN isn't installed on this system yet. Click "
        "Install OpenVPN to fetch it straight from the official "
        "openvpn.net release and set it up automatically — no "
        "browser or manual steps needed.",
        font =(FONT_FAMILY ,9 ,"bold"),fg =WARN ,bg =CARD ,anchor ="w",
        wraplength =400 ,justify ="left").pack (anchor ="w")

        openvpn_btn_row =tk .Frame (openvpn_notice_inner ,bg =CARD )
        openvpn_btn_row .pack (anchor ="w",pady =(8 ,0 ))
        openvpn_status_lbl =tk .Label (openvpn_notice_inner ,text ="",
        font =(FONT_FAMILY ,9 ),fg =MUTED ,bg =CARD ,anchor ="w",
        wraplength =400 ,justify ="left")

        openvpn_install_btn_holder ={"btn":None }

        def do_install_openvpn ():
            if openvpn_install_state ["installing"]:
                return 
            msg =("This downloads OpenVPN straight from its official "
            "site (openvpn.net / swupdate.openvpn.org) and installs "
            "it automatically")
            if os .name !="nt":
                msg +=(" — on this OS that means using your system's "
                "package manager (you may see a password prompt), or "
                "compiling it from source if no package manager is "
                "available")
            msg +=". It can take a few minutes. Continue?"
            if not messagebox .askyesno ("Install OpenVPN",msg ,parent =win ):
                return 

            openvpn_install_state ["installing"]=True 
            openvpn_install_btn_holder ["btn"].set_disabled (True )
            openvpn_install_btn_holder ["btn"].config (text ="Installing…")
            openvpn_status_lbl .config (text ="Starting install…",fg =MUTED )
            openvpn_status_lbl .pack (anchor ="w",pady =(6 ,0 ))

            def on_line (line ):
                def ui ():
                    if win .winfo_exists ():
                        openvpn_status_lbl .config (text =line ,fg =MUTED )
                self .after (0 ,ui )

            def worker ():
                ok ,msg2 =install_openvpn (line_callback =on_line )
                def done ():
                    openvpn_install_state ["installing"]=False 
                    if not win .winfo_exists ():
                        return 
                    openvpn_install_btn_holder ["btn"].config (text ="⚙  Install OpenVPN")
                    openvpn_install_btn_holder ["btn"].set_disabled (False )
                    if ok :
                        openvpn_install_state ["available"]=True 
                        openvpn_status_lbl .config (text =f"✅  {msg2 }",fg ="#3ddc97")
                        openvpn_notice .pack_forget ()
                        Toast (self ,"OpenVPN Installed ✅",msg2 ,ok =True )
                    else :
                        openvpn_status_lbl .config (text =f"🚫  {msg2 }",fg =DANGER )
                        Toast (self ,"Install Failed",msg2 ,ok =False )
                self .after (0 ,done )
            threading .Thread (target =worker ,daemon =True ).start ()

        openvpn_install_btn_holder ["btn"]=FlatBtn (openvpn_btn_row ,
        text ="⚙  Install OpenVPN",base_bg =CARD ,hover_bg =lighten (CARD ,.08 ),
        fg =WARN ,ghost =True ,border_col =WARN ,command =do_install_openvpn ,
        radius =12 ,height =30 ,width =160 ,font =(FONT_FAMILY ,9 ,"bold"))
        openvpn_install_btn_holder ["btn"].pack (side ="left",padx =(0 ,8 ))

        FlatBtn (openvpn_btn_row ,text ="Manual Download",base_bg =CARD ,
        hover_bg =lighten (CARD ,.08 ),fg =MUTED ,ghost =True ,
        border_col =lighten (BORDER ,.2 ),
        ...
        ip_frame =tk .Frame (body ,bg =TOOLBAR ,highlightthickness =1 ,
        highlightbackground =BORDER )
        ip_frame .pack (fill ="x",pady =(20 ,0 ))
        ip_inner =tk .Frame (ip_frame ,bg =TOOLBAR ,padx =14 ,pady =12 )
        ip_inner .pack (fill ="x")
        ip_lbl =tk .Label (ip_inner ,text ="📍  Your IP: checking…",
        font =(FONT_FAMILY ,12 ,"bold"),fg =MUTED ,bg =TOOLBAR ,
        anchor ="w",justify ="left",wraplength =380 )
        ip_lbl .pack (anchor ="w")

        rotate_frame =tk .Frame (body ,bg =BG )
        rotate_frame .pack (fill ="x",pady =(18 ,0 ))
        tk .Label (rotate_frame ,text ="🔗  VPN Chain (Auto-Rotate)",
        font =(FONT_FAMILY ,10 ,"bold"),fg =TEXT ,bg =BG ,anchor ="w"
        ).pack (anchor ="w")
        tk .Label (rotate_frame ,
        text ="Hops to a new random relay from the free VPN Gate list "
        "on a timer, chaining you through a fresh IP automatically. "
        "OpenVPN / VPN Gate relays only — connect through one above, "
        "then turn this on.",
        font =(FONT_FAMILY ,9 ),fg =MUTED ,bg =BG ,anchor ="w",
        wraplength =405 ,justify ="left").pack (anchor ="w",pady =(2 ,8 ))

        _rcfg =load_vpn_rotate_config ()
        rotate_ui_state ={"interval":max (5 ,min (240 ,int (
        _rcfg .get ("interval_min")or self ._vpn_rotate_interval_min or 30 )))}

        controls_row =FlowRow (rotate_frame ,bg =BG ,gap_x =8 ,gap_y =8 )
        controls_row .pack (fill ="x")
        CTRL_H =30 

        rotate_every_lbl =tk .Label (controls_row ,text ="Rotate every",
        font =(FONT_FAMILY ,10 ),fg =TEXT ,bg =BG )
        controls_row .add (rotate_every_lbl ,width =78 ,height =CTRL_H )

        interval_lbl =tk .Label (controls_row ,
        text =f"{rotate_ui_state ['interval']} min",
        font =(FONT_FAMILY ,10 ,"bold"),fg =ACCENT ,bg =BG )

        def bump_interval (delta ):
            rotate_ui_state ["interval"]=max (5 ,min (240 ,
            rotate_ui_state ["interval"]+delta ))
            interval_lbl .config (text =f"{rotate_ui_state ['interval']} min")
            if self ._vpn_rotate_active :
                self ._vpn_rotate_interval_min =rotate_ui_state ["interval"]

        minus_btn =FlatBtn (controls_row ,text ="－",base_bg =TOOLBAR ,
        hover_bg =lighten (TOOLBAR ,.08 ),fg =TEXT ,ghost =True ,
        border_col =lighten (BORDER ,.35 ),radius =12 ,
        font =(FONT_FAMILY ,11 ,"bold"),command =lambda :bump_interval (-5 ))
        controls_row .add (minus_btn ,width =30 ,height =CTRL_H )
        controls_row .add (interval_lbl ,width =54 ,height =CTRL_H )
        plus_btn =FlatBtn (controls_row ,text ="＋",base_bg =TOOLBAR ,
        hover_bg =lighten (TOOLBAR ,.08 ),fg =TEXT ,ghost =True ,
        border_col =lighten (BORDER ,.35 ),radius =12 ,
        font =(FONT_FAMILY ,11 ,"bold"),command =lambda :bump_interval (5 ))
        controls_row .add (plus_btn ,width =30 ,height =CTRL_H )

        def do_toggle_rotate ():
            if not self ._vpn_rotate_active :
                if kind_state ["kind"]!="openvpn"or not vpn_is_active ():
                    messagebox .showwarning ("Connect First",
                    "Connect through an OpenVPN / VPN Gate relay first, "
                    "then turn the chain on.",parent =win )
                    return 
                self ._start_vpn_rotation (rotate_ui_state ["interval"])
            else :
                self ._stop_vpn_rotation ()
            refresh_rotate_ui ()

        rotate_toggle_btn =FlatBtn (controls_row ,text ="🔗  Chain: OFF",
        base_bg =TOOLBAR ,hover_bg =lighten (TOOLBAR ,.08 ),fg =MUTED ,ghost =True ,
        border_col =lighten (BORDER ,.35 ),radius =14 ,
        font =(FONT_FAMILY ,10 ,"bold"),command =do_toggle_rotate )
        controls_row .add (rotate_toggle_btn ,width =130 ,height =CTRL_H +2 )

        def do_rotate_now ():
            if not self ._vpn_rotate_active :
                return 
            self ._vpn_rotate_next_at =time .time ()
            self ._perform_vpn_rotation ()

        rotate_now_btn =FlatBtn (controls_row ,text ="Rotate Now",
        base_bg =TOOLBAR ,hover_bg =lighten (TOOLBAR ,.08 ),fg =TEXT ,ghost =True ,
        border_col =lighten (BORDER ,.35 ),radius =14 ,
        font =(FONT_FAMILY ,10 ,"bold"),command =do_rotate_now )
        controls_row .add (rotate_now_btn ,width =100 ,height =CTRL_H +2 )

        rotate_status_lbl =tk .Label (rotate_frame ,text ="",
        font =(FONT_FAMILY ,9 ),fg =MUTED ,bg =BG ,anchor ="w")
        rotate_status_lbl .pack (anchor ="w",pady =(8 ,0 ))

        def refresh_rotate_ui ():
            if not rotate_toggle_btn .winfo_exists ():
                return 
            active =self ._vpn_rotate_active 
            rotate_toggle_btn .config (
            text =("🔗  Chain: ON"if active else "🔗  Chain: OFF"),
            fg =("#3ddc97"if active else MUTED ))
            rotate_now_btn .set_disabled (not active )
            if active :
                remaining =max (0 ,int (self ._vpn_rotate_next_at -time .time ()))
                m ,s =divmod (remaining ,60 )
                rotate_status_lbl .config (
                text =f"⛓  Chain running — next hop in {m }m {s :02d}s",
                fg ="#3ddc97")
            elif kind_state ["kind"]!="openvpn":
                rotate_status_lbl .config (
                text ="Switch to OpenVPN and connect via a VPN Gate "
                "relay to enable the chain.",fg =MUTED )
            elif not vpn_is_active ():
                rotate_status_lbl .config (
                text ="Off — connect first, then turn the chain on.",
                fg =MUTED )
            else :
                rotate_status_lbl .config (
                text ="Off — turn it on to start auto-rotating relays.",
                fg =MUTED )

        def rotate_ui_tick ():
            if not win .winfo_exists ()or not self .winfo_exists ():
                return 
            refresh_rotate_ui ()
            win .after (1000 ,rotate_ui_tick )

        ip_cache ={"key":None ,"text":None ,"fg":None }

        def refresh_ip (force =False ):
            active =vpn_is_active ()
            key =(active ,kind_state ["kind"],path_state ["path"])
            if not force and ip_cache .get ("key")==key and ip_cache .get ("text"):
                ip_lbl .config (text =ip_cache ["text"],fg =ip_cache ["fg"])
                return 
            ip_lbl .config (text ="📍  Checking your IP…",fg =MUTED )
            ip_op ={"done":False ,"loc":None }
            def worker ():
                ip_op ["loc"]=fetch_vpn_location ()
                ip_op ["done"]=True 
            def done ():
                if not ip_lbl .winfo_exists ():
                    return 
                loc =ip_op ["loc"]
                now_active =vpn_is_active ()
                if loc and loc .get ("ip"):
                    place =", ".join (p for p in (loc .get ("city"),loc .get ("country"))if p )
                    prefix ="🔒  VPN active — "if now_active else "🌐  "
                    text =f"{prefix }Your IP: {loc ['ip']}"
                    if place :
                        text +=f" ({place })"
                    fg ="#3ddc97"if now_active else TEXT 
                    ip_cache ["key"]=(now_active ,kind_state ["kind"],path_state ["path"])
                    ip_cache ["text"]=text 
                    ip_cache ["fg"]=fg 
                    ip_lbl .config (text =text ,fg =fg )
                else :
                    ip_lbl .config (text ="📍  Your IP: unavailable",fg =MUTED )
            def check_ip_op ():
                if ip_op ["done"]:
                    done ()
                    return 
                if ip_lbl .winfo_exists ():
                    self .after (50 ,check_ip_op )
            threading .Thread (target =worker ,daemon =True ).start ()
            self .after (50 ,check_ip_op )

        def refresh_status ():
            if vpn_is_active ():
                connect_btn .config (text ="🔓  Disconnect",fg ="#ff6b6b")
            else :
                connect_btn .config (text ="🔒  Connect",fg =BG )
            refresh_ip (force =True )
            self ._refresh_vpn_btn_label ()
            refresh_rotate_ui ()
        ...
        def open_task_manager ():
            ...
            tbody =tk .Frame (tw ,bg =BG ,padx =22 ,pady =18 )
            tbody .pack (fill ="both",expand =True )

            tk .Label (tbody ,text ="🧹  VPN Tasks",font =(FONT_FAMILY ,16 ,"bold"),
            fg =TEXT ,bg =BG ,anchor ="w").pack (anchor ="w",pady =(0 ,4 ))
            tk .Label (tbody ,
            text ="Every running process with \"VPN\" in its name. "
            "End one to cut that connection — the status above will "
            "update once nothing VPN-related is left running.",
            font =(FONT_FAMILY ,9 ),fg =MUTED ,bg =BG ,anchor ="w",
            wraplength =390 ,justify ="left").pack (anchor ="w",pady =(0 ,12 ))

            list_frame =tk .Frame (tbody ,bg =BG )
            list_frame .pack (fill ="both",expand =True )

            tw_state ={"open":True ,"pending_id":None }

            def end_one (pid ,end_btn ):
                end_btn .set_disabled (True )
                end_btn .config (text ="Ending…")
                result ={"done":False ,"ok":None ,"msg":None }

                def worker ():
                    ok ,msg =_end_task_by_pid (pid )
                    result ["ok"]=ok 
                    result ["msg"]=msg 
                    result ["done"]=True 
                ...
                def check ():
                    ...
                    if result ["done"]:
                        if result ["ok"]:
                            ...
                            end_btn .set_disabled (False )
                            end_btn .config (text ="End Task")
                            messagebox .showerror ("Couldn't End Task",
                            result ["msg"]or "Unknown error.",parent =tw )
                        return 
                    tw .after (50 ,check )
            ...
            def render_tasks ():
                for w in list_frame .winfo_children ():
                    w .destroy ()
                tasks =_list_vpn_named_tasks ()
                if not tasks :
                    tk .Label (list_frame ,text ="No VPN tasks found running.",
                    font =(FONT_FAMILY ,10 ),fg =MUTED ,bg =BG ).pack (pady =24 )
                    return 
                for t in tasks :
                    row =tk .Frame (list_frame ,bg =TOOLBAR ,highlightthickness =1 ,
                    highlightbackground =BORDER )
                    row .pack (fill ="x",pady =(0 ,6 ))
                    inner =tk .Frame (row ,bg =TOOLBAR ,padx =10 ,pady =8 )
                    inner .pack (fill ="x")
                    end_btn =FlatBtn (inner ,text ="End Task",base_bg ="#ff6b6b",
                    hover_bg =lighten ("#ff6b6b",.1 ),fg =BG ,
                    font =(FONT_FAMILY ,9 ,"bold"),radius =12 ,height =26 ,width =88 ,
                    command =None )
                    end_btn ._cmd =lambda pid =t ["pid"],b =end_btn :end_one (pid ,b )
                    end_btn .pack (side ="right")
                    text_col =tk .Frame (inner ,bg =TOOLBAR )
                    text_col .pack (side ="left",fill ="x",expand =True )
                    tk .Label (text_col ,text =t ["name"],font =(FONT_FAMILY ,10 ,"bold"),
                    fg =TEXT ,bg =TOOLBAR ,anchor ="w").pack (anchor ="w")
                    tk .Label (text_col ,text =f"PID {t ['pid']}",font =(FONT_FAMILY ,9 ),
                    fg =MUTED ,bg =TOOLBAR ,anchor ="w").pack (anchor ="w")

            def refresh_loop ():
                if not tw_state ["open"]:
                    return 
                render_tasks ()
                if tw_state ["open"]and tw .winfo_exists ():
                    tw_state ["pending_id"]=tw .after (3000 ,refresh_loop )

            def close_tw ():
                tw_state ["open"]=False 
                if tw_state ["pending_id"]is not None :
                    try :tw .after_cancel (tw_state ["pending_id"])
                    except Exception :pass 
                refresh_status ()
                tw .destroy ()

            btn_row2 =tk .Frame (tbody ,bg =BG )
            btn_row2 .pack (fill ="x",pady =(14 ,0 ))
            FlatBtn (btn_row2 ,text ="Refresh",base_bg =TOOLBAR ,
            hover_bg =lighten (TOOLBAR ,.08 ),fg =TEXT ,ghost =True ,
            border_col =lighten (BORDER ,.35 ),radius =14 ,
            font =(FONT_FAMILY ,10 ,"bold"),command =render_tasks ,
            height =32 ,width =100 ).pack (side ="left")
            FlatBtn (btn_row2 ,text ="Close",base_bg =BORDER ,fg =TEXT ,
            command =close_tw ,radius =14 ,height =32 ,width =100 
            ).pack (side ="right")
            tw .protocol ("WM_DELETE_WINDOW",close_tw )
        ...
        def do_toggle ():
            ...
            op_state ={"done":False ,"ok":None ,"msg":None }
                ...
                if win .winfo_exists ():
                    ...
                    if ok :
                        ...
                        if going_up :
                            ...
                            connect_btn .config (text ="🔓  Disconnect",fg ="#ff6b6b")
                        ip_lbl .config (text =f"⚠️  {msg }",fg ="#ff6b6b")
                    if not going_up and ok and self ._vpn_rotate_active :
                        self ._stop_vpn_rotation (silent =True )
                    refresh_ip (force =True )
                    self ._refresh_vpn_btn_label ()
                    refresh_rotate_ui ()
            ...
            def check_op ():
                if not poll_state ["open"]:
                    poll_state ["done_id"]=None 
                    return 
                if op_state ["done"]:
                    poll_state ["done_id"]=None 
                    done (op_state ["ok"],op_state ["msg"])
                    return 
                if win .winfo_exists ():
                    poll_state ["done_id"]=self .after (50 ,check_op )
                else :
                    poll_state ["done_id"]=None 
        ...
        poll_state ={"open":True ,"pending_id":None ,"last":None ,"done_id":None}
        _TRANSITIONAL ={"Connecting…","Disconnecting…"}

        def close ():
            poll_state ["open"]=False
            if poll_state ["pending_id"]is not None :
                try :self .after_cancel (poll_state ["pending_id"])
                except Exception :pass
            if poll_state ["done_id"]is not None :
                try :self .after_cancel (poll_state ["done_id"])
                except Exception :pass
            win .destroy ()

        FlatBtn (btn_row ,text ="Close",base_bg =BORDER ,fg =TEXT ,
        command =close ,radius =18 ,height =38 ,width =90 
        ).pack (side ="left",padx =(8 ,0 ))
        win .protocol ("WM_DELETE_WINDOW",close )

        tasks_row =tk .Frame (body ,bg =BG )
        tasks_row .pack (fill ="x",pady =(10 ,0 ))
        FlatBtn (tasks_row ,text ="🧹  VPN Tasks",base_bg =TOOLBAR ,
        hover_bg =lighten (TOOLBAR ,.08 ),fg =TEXT ,ghost =True ,
        border_col =lighten (BORDER ,.35 ),radius =18 ,
        font =(FONT_FAMILY ,11 ,"bold"),command =open_task_manager ,
        height =36 ,width =160 ).pack (side ="left")

        def poll_real_state ():
            if not poll_state ["open"]:
                return 
            if connect_btn ._text not in _TRANSITIONAL :
                real =vpn_is_active ()
                if real !=poll_state ["last"]:
                    poll_state ["last"]=real 
                    refresh_status ()
            if poll_state["open"] and win.winfo_exists() and self.winfo_exists():
                poll_state ["pending_id"]=self .after (2500 ,poll_real_state )

        refresh_kind_buttons ()
        refresh_status ()
        refresh_rotate_ui ()
        rotate_ui_tick ()
        poll_state ["last"]=vpn_is_active ()
        if poll_state["open"] and win.winfo_exists() and self.winfo_exists():
            poll_state ["pending_id"]=self .after (2500 ,poll_real_state )
